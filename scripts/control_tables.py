...
def write_control_table(path: str | Path, new: pd.DataFrame, *, generator: str,
                        key: str = KEY, backup: bool = True,
                        drop_conditions: list[str] | None = None) -> pd.DataFrame:
...
def audit_control_table(path: str | Path) -> dict:
    """Provenance summary for one table: {n_rows, conditions, written_at:[...], generator:[...], mixed: bool}."""
    path = Path(path)
    df = pd.read_csv(path)
    ...
    return {"path": str(path), "n_rows": len(df),
            "conditions": sorted(df[KEY].dropna().unique()) if KEY in df else [],
            "written_at": dates, "generator": gens,
            "mixed": len(dates) > 1 or len(gens) > 1}


def snapshot_control_tables(label: str, runs: list[str], *, reason: str,
                            root: str | Path = "results") -> Path:
    """Copy every per-layer control table from `runs` into results/_control_scores_<label>/ and write a README recording `reason`. Returns the snapshot dir."""
    root = Path(root)
    ...
    (snap / "README.md").write_text(
        ...
        f"## Reason\n\n{reason}\n\n"
        f"## Layout\n\n`<run>/blocks<L>_control_{{within,between}}_scores.csv` — verbatim copies of the\n"
        f"tables as they stood at snapshot time.\n\n"
        f"Snapshots are labelled rather than timestamped so the reason stays attached to the data;\n"

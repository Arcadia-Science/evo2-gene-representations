...
def main() -> None:
    ...
    ap.add_argument(
        "--csv", required=True, help="tidy within_family_vs_layer*.csv (layer, metric, family, rho)"
    )
    ...
    if args.families:
        if args.out_suffix == "per_family":
            raise SystemExit(
                "--families needs --out-suffix (e.g. 'zoom3'); the default name is "
        ...
        if missing:
            raise SystemExit(
                f"not in {args.csv}: {missing}\navailable: {sorted(df['family'].dropna().unique())}"
            )
            ...
            rows.append(
                {
                    ...
                    "metric_b": b,
                    "n_layers": int(ok.sum()),
                    "spearman_across_layers": (
                        spearmanr(w[a][ok], w[b][ok]).statistic if ok.sum() >= 4 else np.nan
                    ),
                }
            )
    ...
    med = (
        pairs.groupby(["metric_a", "metric_b"])["spearman_across_layers"]
    ...
    # are sized to the figure, and at sc=1.8 they swallowed a 3-panel zoom. Panel-level type
    # (titles,
    ...
    fig.legend(
        ...
        frameon=False,
        fontsize=round(7 * fsc),
        bbox_to_anchor=(0.5, -0.34 / (args.panel_h * nrows)),
    )
    fig.suptitle(
        f"{args.title} — within-family ρ across layers, per family",
        fontsize=round(11 * fsc),
        fontweight="bold",
        y=1.0,
    )

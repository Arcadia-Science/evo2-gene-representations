...
def mantel_within(geo_sub: np.ndarray, D: np.ndarray, n_perms: int, seed: int,
                  two_sided: bool = True) -> dict | None:
    ...
    for _ in range(n_perms):
        ...
        if (abs(r) >= abs(obs_rho)) if two_sided else (r >= obs_rho):
            count += 1
    return {"rho": float(obs_rho), "p_naive": float(p_naive),
            "p_mantel": (count + 1) / (n_perms + 1), "n_pairs": int(ok.sum()),
...
def bootstrap_ci(geo_sub: np.ndarray, D: np.ndarray, n_boot: int, seed: int,
...
def load_panel(run_dir: Path):
    ...
    hits = sorted(run_dir.glob("*_geodesic_labeled.csv")) + \
        sorted(run_dir.glob("*_geodesic_labeled.csv.gz"))
    if not hits:
...
def main() -> None:
    ...
    print(f"{'family':<28}{'baseline':<11}{'n':>5}{'rho':>8}{'p_naive':>11}{'p_mantel':>10}"
          f"{'95% CI':>18}")
    ...
    for fam in families:
        ...
        for bname in sorted(mats):
            res = mantel_within(geo_sub, mats[bname], args.n_perms, args.seed,
                                two_sided=not args.one_sided)
            if res is None:
                continue
            lo, hi = bootstrap_ci(geo_sub, mats[bname], args.n_boot, args.seed + 1)
            rows.append({"family": fam, "baseline": bname, "n_members": len(members), **res,
                         "ci_lo": lo, "ci_hi": hi,
                         "ci_excludes_zero": bool(np.isfinite(lo) and lo * hi > 0)})
            print(f"{fam:<28}{bname:<11}{len(members):>5}{res['rho']:>+8.3f}"
    ...
    for bname, g in t.groupby("baseline"):
        ...
        print(f"{bname:<11} {len(gt):>3}/{len(g)} testable | mean ρ {g.rho.mean():+.3f} | "
    ...
    if len(lost):
        ...
        for _, r in lost.iterrows():
            print(f"  {r.family:<28} {r.baseline:<10} ρ {r.rho:+.3f}   "
    ...
    if len(under):
        ...
        print("  " + ", ".join(f"{f} (n={int(under[under.family == f].n_members.iloc[0])}, "

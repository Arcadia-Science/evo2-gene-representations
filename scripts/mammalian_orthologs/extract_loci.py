"""
Extract the transcript-span genomic locus (+ CDS) for every resolved mammalian ortholog, from each
species' own Ensembl assembly, and apply per-locus QC.
"""
...
def fetch_locus(species: str, gene_id: str) -> dict | None:
    ...
    ov = _get(
        f"/overlap/region/{species}/{chrom}:{tx_start}..{tx_end}?feature=gene;content-type=application/json"
    )
    n_overlap = 0
    if isinstance(ov, list):
        n_overlap = sum(
            1 for f in ov if f.get("id") != gene_id and f.get("biotype") == "protein_coding"
        )
    return {
        "species": species,
        "gene_id": gene_id,
        ...
        "biotype": biotype,
        "n_exons": len(exons),
        "locus_seq": (locus_seq or "").upper(),
        "cds_seq": (cds_seq or "").upper(),
...
def main() -> None:
    ...
    print(
        f"{len(work)} loci to extract ({work['human_gene'].nunique()} groups, "
        f"{work['family'].nunique()} families)",
        flush=True,
    )
    ...
    if missing_h:
        print(
            f"resolving {len(missing_h)} human gene ids (parallel, workers={args.workers})...",
            flush=True,
        )
        with ThreadPoolExecutor(max_workers=args.workers) as ex:
            for g, gid in zip(missing_h, ex.map(_resolve_human_gene_id, missing_h), strict=False):
                if gid:  # only persist RESOLVED ids; None (Ensembl outage) retries next run
                    human_ids[g] = gid
        hid_cache.write_text(json.dumps(human_ids))
    print(
        f"human gene ids ready ({sum(1 for v in human_ids.values() if v)}/"
        f"{len(human_ids)} resolved)",
        flush=True,
    )
    ...
    def _extract(row):
        ...
        if cache.exists():
            return
        gid = (
            human_ids.get(row.human_gene) if row.species == "homo_sapiens" else row.ortholog_gene_id
        )

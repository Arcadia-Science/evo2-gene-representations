...
def main() -> None:
    ...
    for f in tqdm(files, desc="recovering CDS positions"):
        try:
            ...
            rows.append(dict(key=f.stem, status="unreadable"))
            continue
        ...
        if not locus or not cds:
            rows.append(dict(key=key, family=d.get("family"), status="no_seq"))
            continue
        ...
        if pos is None:
            ...
            continue
        # exact reconstruction: count must equal the spliced CDS length, indices strictly
        # increasing.

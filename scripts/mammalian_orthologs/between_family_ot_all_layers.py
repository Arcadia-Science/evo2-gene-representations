"""Between-family control ladder scored by all-gene Wasserstein (W2) at every layer — the W2 counterpart of the run's `control_rho_by_layer.csv` figures."""
...
LADDER = ["gc_match", "dinuc_shuffle", "kmer4_shuffle", "kmer6_shuffle",
...
NT_IDENTITY = {"gc_match": 0.257, "dinuc_shuffle": 0.262, "kmer4_shuffle": 0.266,
               "kmer6_shuffle": 0.273, "synonymous_recode": 0.771, "missense_subset": 0.876}
AA_IDENTITY = {"gc_match": 0.058, "dinuc_shuffle": 0.061, "kmer4_shuffle": 0.067,
               "kmer6_shuffle": 0.076, "synonymous_recode": 1.000, "missense_subset": 0.723}
...
def all_layer_w2(arm_dir: str, shared, fam_arr, fam_order, layers, workers: int) -> dict:
    ...
    if stack is None or len(present) != len(shared):
        raise RuntimeError(f"{arm_dir}: expected {len(shared)} loci, got "
                           f"{0 if stack is None else len(present)}")
    ...
    del stack
    print(f"    load {load_s:5.1f}s  +  {len(layers)} layers in "
          f"{time.perf_counter() - t0 - load_s:6.1f}s", flush=True)
...
def main() -> None:
    ...
    fam_of = dict(zip(man.group + "__" + man.species, man.family))
    ...
    shared = [k for k, m in zip(shared, keep) if m]
    ...
    for c in args.controls:
        ...
        for L in args.layers:
            rows.append({"layer": L, "condition": c, "metric": "wasserstein",
                         "manifest": args.manifest, "n_loci": len(shared),
                         "n_families": len(fam_order), "n_target": n_target,
                         "n_family_pairs": len(fam_order) * (len(fam_order) - 1) // 2,
                         "between_preservation": spearmanr(nat[L], w[L]).correlation,
                         "nt_identity": NT_IDENTITY[c], "aa_identity": AA_IDENTITY[c]})
    ...
    if MISSENSE in piv and RECODE in piv:
        ...
        if len(early) and len(late):
            print(f"    blocks 0-9   mean {early.mean():+.4f}  lower in "
                  f"{(early < 0).sum()}/{len(early)}")
            print(f"    blocks 10-27 mean {late.mean():+.4f}  lower in {(late < 0).sum()}/{len(late)}")
...
def figure(t: pd.DataFrame, path: Path, n_shared: int, n_fams: int) -> None:
    ...
    for c in [x for x in LADDER if x in set(t.condition)]:
        ...
        ax.plot(s.layer, s.between_preservation, "-o", ms=3,
                lw=2.2 if c == MISSENSE else 1.3, color=col[c],
    ...
    fig.suptitle(f"Between-family control ladder by Wasserstein, {n_shared:,} loci / {n_fams} "
                 + (f"families. INTERIM: {n_target - n_shared:,} loci of a control cache still "
                    f"filling." if partial else "families."), fontsize=8.5)

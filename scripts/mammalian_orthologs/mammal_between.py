...
from between_family_baselines import (  # noqa: E402
    PFAM_ACCESSIONS, go_matrices, cofactor_matrix, ec_matrix,
)
...
AXIS = {"pfam_jsd": "1_homology", "cofactor": "2_mechanism", "ec_number": "2_mechanism",
        "go_mf": "2_mechanism", "gc_content": "control", "kmer": "control"}
...
def load_embedded(arm: str, manifest: str = "complete_manifest.csv"):
    ...
    man = pd.read_csv(OUT / manifest); man["key"] = man.group + "__" + man.species
    vecs, rows = [], []
...
def load_cds() -> dict[str, str]:
    ...
    for fa in (OUT / "seqs" / "complete" / "cds").glob("*.fasta"):
        ...
        for line in fa.read_text().splitlines():
            if line.startswith(">"):
                if hid: seqs[hid] = "".join(chunk)
                p = line[1:].split("|"); hid = f"{p[0]}__{p[1]}"; chunk = []
            elif line.strip():
...
def gc_between(meta, cds, fams):
    def gc(s): s = s.upper(); return (s.count("G") + s.count("C")) / max(1, len(s))
...
def main() -> None:
    ...
    ap.add_argument("--tag", default="",
                    help="suffix appended to the run-dir arm name (e.g. _400)")
    ...
    sub_idx = _cols_for(meta, args.arm, fams, args.manifest)   # stack cols for kept families, meta order
    ...
    print("computing transferable axes (Pfam-JSD, GO — cached fetch; k-mer, GC on panel CDS)...", flush=True)
    axes = {
    ...
    for L in args.layers:
        ...
        run = sweep_root / f"blocks{L}"; run.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(cen, index=fams, columns=fams).to_csv(run / "evo2_mammal_centroid_distances.csv")
        rows = []
    ...
    print(f"wrote between_family_baseline_scores.csv to {len(args.layers)} run dirs -> {sweep_root}", flush=True)
...
def _cols_for(meta, arm, fams, manifest="complete_manifest.csv"):
    ...
    full = pd.read_csv(OUT / manifest); full["key"] = full.group + "__" + full.species
    cache = CACHE_ROOT / arm
    present = [(k, f) for k, f in zip(full["key"], full["family"]) if (cache / f"{k}.npy").exists()]

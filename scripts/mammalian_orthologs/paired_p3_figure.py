...
# Evo2 taps the last block twice and blocks 28+ blow up in norm on this arm; see the module docstring.
...
def verify_identity() -> dict:
    ...
    for key in sorted(nat):
        src = nat[key]
        syn = paired_p3(src, usage[fam_of[key]],
                        random.Random(zlib.crc32(f"{SYN}:{key}".encode())), "synonymous")
        mis = paired_p3(src, {},
                        random.Random(zlib.crc32(f"{MIS}:{key}".encode())), "missense")
    ...
    return {"n": len(nat), "same_positions": same_pos,
...
def load_between() -> pd.DataFrame:
    rows = []
    for L in LAYERS:
        s = pd.read_csv(RUN / f"blocks{L}/controls/control_between_scores.csv").set_index("condition")
        rows.append({"layer": L, **{c: s.loc[c, "rho_vs_natural_between"]
                                   for c in (SYN, MIS, FLOOR) if c in s.index}})
...
def load_per_family(layer: int) -> pd.DataFrame:
    ...
    return d.pivot_table(index="family", columns="condition",
                         values="rho_geodesic_vs_natural").dropna()
...
def paired_stats(layers: list[int]) -> pd.DataFrame:
    ...
    for L in layers:
        p = load_per_family(L)
        diff = p[MIS] - p[SYN]
        rows.append({"layer": L, "n": len(diff), "mean_gap": diff.mean(),
...
def figure(within: pd.DataFrame, between: pd.DataFrame, out: Path) -> None:
    ...
    for ax, tbl, ttl in (
        ...
        if not pub.is_on():
            ax.legend(title="Control", fontsize=6, loc="lower left", frameon=False,
                      alignment="left")
...
def _key_rule(fig, axes, colour) -> None:
    ...
    for ax in axes:
        ...
        ax.add_line(Line2D([x0, x1], [y, y], transform=ax.transAxes, color=colour,
...
def main() -> None:
    ...
    ap.add_argument("--peak-layer", type=int, default=18,
    ...
    if ident:
        print(f"identity: identical edited positions {ident['same_positions']:,}/{ident['n']:,}, "
    ...
    print(f"within  gap: mean {w.mean():+.4f}, lower in {(w < 0).sum()}/{len(w)} layers, "
          f"deepest {w.min():+.4f} @ block {w.idxmin()}")
    print(f"between gap: mean {b.mean():+.4f}, lower in {(b < 0).sum()}/{len(b)} layers, "
          f"deepest {b.min():+.4f} @ block {b.idxmin()}")

    row = stats_tbl[stats_tbl.layer == a.peak_layer].iloc[0]
    print(f"block {a.peak_layer}: missense lower in "

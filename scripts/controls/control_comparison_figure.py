...
from plot_utils import control_preservation_figure, grouped_rho_bars  # noqa: E402
...
def preservation_figure(out_dir: Path, family_order: list[str]) -> None:
    ...
    if "rho_geodesic_vs_natural" not in win.columns:
        ...
        return
    within_by = {
        cond: dict(zip(g["family"], g["rho_geodesic_vs_natural"], strict=False))
    ...
    if btw_p.exists():
        ...
        if "rho_vs_natural_centroid" in btw.columns:
            between_by = dict(zip(btw["condition"], btw["rho_vs_natural_centroid"], strict=False))
    ...
    ok = control_preservation_figure(
        str(out_dir / "control_preservation"),
        family_order,
        display,
        within_by,
        between_by_condition=between_by,
        title_left="Composition controls: is the within-family geometry preserved? "
        "(Evo2 gene families)",
    )
    print(
        f"Saved {out_dir}/control_preservation.{{pdf,png}}"
        if ok
...
def main() -> None:
    ...
    ap.add_argument(
        "--out",
        default=None,
        help="Output basename (default: <natural-run>/controls/control_comparison)",
    )
    args = ap.parse_args()
    ...
    fig, (axL, axR) = plt.subplots(
        1, 2, figsize=(20, 6.5), dpi=200, gridspec_kw={"width_ratios": [3, 1]}
    )
    grouped_rho_bars(
        axL,
        family_order,
    ...
    for x, m in zip(xs, means, strict=False):
        ...
        [
            {
                ...
                "mean_rho_taxonomy": m,
                "std_rho_taxonomy": sd,
                "pct_of_natural": (m / nat_mean if nat_mean else np.nan),
            }
            for lbl, m, sd, _ in summary
        ]
    )

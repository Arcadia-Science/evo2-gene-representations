"""How much of the source sequence does each composition control retain, and does the ladder's rho just track that? Sequence-level only — no model, no GPU."""
...
LADDER = ["kmer6_shuffle", "kmer4_shuffle", "codon_shuffle", "dinuc_shuffle", "gc_match",
...
def pair_metrics(a: str, b: str, in_frame: bool) -> dict:
    ...
    out = {
        ...
        "edit_identity": 1.0 - edlib.align(a, b, task="distance", mode="NW")["editDistance"]
        / max(len(a), len(b), 1),
    ...
    if in_frame:
        ...
        if ncod:
            ...
            out |= {"p1_identity": float(by_pos[0]), "p2_identity": float(by_pos[1]),
...
def draw_control(control: str, seqs: dict[str, str], fam_of: dict[str, str],
                 seed_tag: str) -> dict[str, str]:
    ...
    for g, s in seqs.items():
        ...
        if control == "gc_match":
            ...
            out[g] = (recoded if control == "synonymous_recode"
                      else mcs.missense_subset(s, recoded, rng))
        else:
...
def load_xkingdom():
    ...
    fams = list(order)
    embedded = {c: _read_fastas(mcs.CONTROL_ROOT / c, fams)
                for c in LADDER if (mcs.CONTROL_ROOT / c).is_dir()}
...
def load_mammal_cdsmask():
    ...
    keys = set.intersection(*[{p.stem for p in (cache / f"transcript_cdsmask_{c}").glob("*__*.npy")}
                             for c in rungs])
...
def measure_identity(panel: str, max_seqs: int | None, verify_on_disk: bool) -> pd.DataFrame:
    ...
    print(f"[{panel}] {len(nat)} sequences, {len(set(fam_of.values()))} families, "
    ...
    for control in rungs:
        ...
        for g, s in nat.items():
            ...
            rows.append({"panel": panel, "condition": control, "key": g, "family": fam_of[g],
                         "primary_from": source_of_primary,
                         **m,
                         **{f"null_{k}": v for k, v in null.items()
                            if k not in ("len_src", "len_delta")},
...
def _applicable_rungs(panel: str, in_frame: bool) -> list[str]:
    ...
    tables = sorted(run.glob(f"{PANELS[panel]['layer_dir'].format(L='*')}/controls/"
                             "control_within_scores.csv"))
    if not tables:
...
def _verify_on_disk(nat: dict, fam_of: dict, embedded: dict) -> None:
    """Confirm the on-disk control FASTAs are the ones make_control_sequences generates, by replaying its
    exact RNG discipline.
    """
    _, order = mcs.load_family_fastas()
    usage = mcs.build_family_codon_usage({f: [nat[g] for _, g in h] for f, h in order.items()
                                          if all(g in nat for _, g in h)})
    for control, disk in embedded.items():
        ...
        print(f"  [verify] {control:18} {same}/{total} on-disk sequences reproduce from SEED="
              f"{mcs.SEED}")
...
def summarise_identity(per_seq: pd.DataFrame) -> pd.DataFrame:
    ...
    num = [c for c in per_seq.columns
           if per_seq[c].dtype.kind == "f" or c in ("len_src", "len_delta")]
...
def load_rho(panel: str) -> pd.DataFrame:
    """Tidy (condition, layer, rho_preservation, rho_recovery) for every layer of one panel, from that panel's own controls/control_within_scores.csv, averaged over families."""
    spec = PANELS[panel]
    run = spec["run"]()
    rows = []
    for d in sorted(run.glob(spec["layer_dir"].format(L="*"))):
        ...
        for cond, grp in df.groupby("condition"):
            rows.append({"panel": panel, "layer": layer, "condition": cond,
        ...
        if "natural" in have:  # some panels already write a natural row; fill its recovery
            for r in rows:
                if r["layer"] == layer and r["condition"] == "natural" and not np.isfinite(
                        r["rho_recovery"]):
                    r["rho_recovery"] = nat_rec
        else:
            rows.append({"panel": panel, "layer": layer, "condition": "natural",
                         "n_families": int(df["family"].nunique()),
                         "rho_preservation": 1.0, "rho_recovery": nat_rec})
    if not rows:
    ...
    for (panel,), ig in ident.groupby(["panel"]):
        ...
        for (layer,), rg in rho[rho["panel"] == panel].groupby(["layer"]):
            ...
            for rho_col in ("rho_preservation", "rho_recovery"):
                ...
                # negative controls on the x-axis itself: both are computable without ever looking at
                ...
                for x_col in ("pos_identity", "edit_identity", "pos_excess", "aa_identity",
                              "null_pos_identity", "pos_identity_chance"):
                    if x_col not in idm.columns:
                        continue
                    conds = [c for c in LADDER if c in idm.index and c in r.index
                             and np.isfinite(idm.loc[c, x_col]) and np.isfinite(r.loc[c, rho_col])]
                    if len(conds) < 3:
                        continue
                    ...
                    out.append({"panel": panel, "layer": layer, "rho_col": rho_col,
                                "identity_metric": x_col, "n_rungs": len(conds),
                                "spearman_all_rungs": rho_all[0], "p_all_rungs": rho_all[1],
                                ...
                                "recode_rho_predicted": pred, "recode_rho_residual": resid,
                                "recode_extrapolation_ranges": extrap})
    return pd.DataFrame(out)


def within_rung_stats(per_seq: pd.DataFrame, panel: str, layer: int) -> pd.DataFrame:
    """The test that does not rely on comparing rungs to each other: WITHIN one rung, across families, does a family whose sequences retained more identity get a higher ρ?"""
    spec = PANELS[panel]
    ...
    fam_id = (per_seq.groupby(["condition", "family"])[["pos_identity", "edit_identity"]]
              .mean().reset_index())
    ...
    for cond, g in m.groupby("condition"):
        for rho_col in ["rho_geodesic_vs_natural"] + ([rec] if rec else []):
            ...
            out.append({"panel": panel, "layer": layer, "condition": cond, "rho_col": rho_col,
                        "n_families": len(ok), "spearman_identity_vs_rho": s[0], "p": s[1]})
    return pd.DataFrame(out)
...
def _scatter_rho_vs_identity(lo, hi, panel: str, idm: pd.DataFrame, rl: pd.DataFrame,
                             stats: pd.DataFrame, conds: list[str], layer: int, rho_col: str,
                             title: str) -> None:
    ...
    for ax in (lo, hi):
        for i, c in enumerate(conds):
            ax.scatter(xs[i], ys[i], s=60, color=COLOUR.get(c, acs.CONTROL_FALLBACK), zorder=3,
                       edgecolor=acs.apc.white, linewidth=0.5,
                       marker="D" if c in NESTED_PAIR else "o")
        if np.isfinite(nat_y):
            ax.scatter([1.0], [nat_y], s=60, facecolor="none", zorder=3,
                       edgecolor=COLOUR.get("natural", acs.REFERENCE_LINE), linewidth=1.2)
        if len(nuc) >= 2 and np.ptp(xs[nuc]) > 0:
            m, b = np.polyfit(xs[nuc], ys[nuc], 1)
            ax.plot(np.linspace(0.2, 1.05, 60), m * np.linspace(0.2, 1.05, 60) + b,
                    ls=":", lw=0.7, color=acs.GRID)
            fx = np.linspace(min(xs[nuc]), max(xs[nuc]), 20)
            ax.plot(fx, m * fx + b, ls="-", lw=1.4, color=acs.ANNOTATION)
    if np.isfinite(nat_y):
        hi.annotate("natural\n(reference)", (1.0, nat_y), textcoords="offset points",
                    xytext=(-3, 8), fontsize=5.5, ha="right",
                    color=COLOUR.get("natural", acs.REFERENCE_LINE))
    # Label the cluster on the zoomed axis. The nucleotide rungs can sit within 0.01 of each other in
    ...
    for rank, i in enumerate(sorted(nuc, key=lambda i: ys[i])):
        lo.annotate(LABEL.get(conds[i], conds[i]), xy=(xs[i], ys[i]), xycoords="data",
                    xytext=(0.62, 0.08 + 0.42 * rank / max(len(nuc) - 1, 1)),
                    textcoords="axes fraction", fontsize=5.5, va="center", ha="left",
    ...
    for rank, i in enumerate(ranked):
        if len(ranked) == 1:
            hi.annotate(LABEL.get(conds[i], conds[i]), xy=(xs[i], ys[i]),
                        textcoords="offset points", xytext=(0, 10), fontsize=5.5, ha="center",
                        fontweight="bold")
            continue
        hi.annotate(LABEL.get(conds[i], conds[i]), xy=(xs[i], ys[i]), xycoords="data",
    ...
    for ax, xf in ((lo, 1.0), (hi, 0.0)):   # break marks
        ax.plot([xf, xf], [0, 1], transform=ax.transAxes, clip_on=False, color=acs.apc.black,
                lw=0.8, ls=(0, (2, 3)))
    st = stats[(stats.panel == panel) & (stats.layer == layer) & (stats.rho_col == rho_col)
    ...
    if len(st):
        ...
        lo.text(0.03, 0.97,
                ...
                transform=lo.transAxes, va="top", fontsize=5.5,
...
def figure(panel: str, ident: pd.DataFrame, rho: pd.DataFrame, stats: pd.DataFrame,
           layer: int, out_dir: Path) -> None:
    import matplotlib
    matplotlib.use("Agg")
    ...
    fig.suptitle(
        ...
        f"[{panel}, blocks.{layer}]", fontsize=11, y=0.985)

    # (A) the identity decomposition: identity to source vs the self-pair null vs the chance floor.
    ax = axes[(0, 0)]
    x = np.arange(len(conds))
    w = 0.27
    ax.bar(x - w, [idm.loc[c, "pos_identity"] for c in conds], w,
           color=[COLOUR.get(c, acs.CONTROL_FALLBACK) for c in conds], label="to source")
    ax.bar(x, [idm.loc[c, "null_pos_identity"] for c in conds], w,
           facecolor="none", edgecolor=acs.ANNOTATION, hatch="///", linewidth=0.6,
           label="two independent draws (null)")
    ax.bar(x + w, [idm.loc[c, "pos_identity_chance"] for c in conds], w,
           color=acs.MISSING, label="composition chance")
    ax.set_xticks(x)
    ...
    ax.set_title("A. Identity to source is the control's own\nconstraint, not retained sequence",
                 fontsize=8)
    ax.legend(fontsize=5.5, loc="upper left")
    ax.set_ylim(0, 1.05)

    # (B, C) rho vs identity, with the nucleotide-only fit extended to the nested pair.
    for j, rho_col, name in ((1, "rho_preservation", "vs natural geodesic (PRESERVATION)"),
                             (2, "rho_recovery", f"vs {spec['recovery_name']} (RECOVERY)")):
        lo, hi = broken[j]
        _scatter_rho_vs_identity(lo, hi, panel, idm, rl, stats, conds, layer, rho_col,
    ...
    se = np.array([idm.loc[c, "pos_excess_se"] if "pos_excess_se" in idm.columns else np.nan
                   for c in conds])
    ax.bar(range(len(conds)), ex, color=[COLOUR.get(c, acs.CONTROL_FALLBACK) for c in conds],
           yerr=1.96 * se, error_kw=dict(lw=0.8, capsize=2, ecolor=acs.ANNOTATION))
    ...
    ax.set_ylim(-lim, lim)
    ax.set_title("D. Retained SOURCE identity, once each rung's own\n"
                 "constraint is netted out (±95% CI)", fontsize=8)

    # (E, F) is the monotonicity a layer-specific accident? Spearman per layer.
    for ax, rho_col, tag in ((axes[(1, 1)], "rho_preservation", "E"),
                             (axes[(1, 2)], "rho_recovery", "F")):
        s = stats[(stats.panel == panel) & (stats.rho_col == rho_col)
                  & (stats.identity_metric == "pos_identity")].sort_values("layer")
        if len(s):
            ax.plot(s.layer, s.spearman_all_rungs, "-o", ms=2.5, lw=1.0, color=acs.apc.amber,
                    label="all rungs (recode included)")
            ax.plot(s.layer, s.spearman_nucleotide_rungs, "-s", ms=2.5, lw=1.0, color=acs.apc.dusk,
                    label="nucleotide rungs only")
            s2 = stats[(stats.panel == panel) & (stats.rho_col == rho_col)
                       & (stats.identity_metric == "pos_excess")].sort_values("layer")
            if len(s2):
                ax.plot(s2.layer, s2.spearman_all_rungs, "-^", ms=2.5, lw=1.0, color=acs.CONTROL_FALLBACK,
...
def figure_table(panel: str, ident: pd.DataFrame, rho: pd.DataFrame, stats: pd.DataFrame,
                 layer: int, out_dir: Path) -> None:
    ...
    for c in show:
        ...
        g = lambda k: ("1.000" if nat else                                       # noqa: E731
                       (f"{r[k]:.3f}" if k in r and np.isfinite(r[k]) else "–"))
        rr = lambda k: (f"{rl.loc[c, k]:.3f}" if np.isfinite(rl.loc[c, k]) else "–")  # noqa: E731
        rows.append([LABEL.get(c, c), g("pos_identity"), g("null_pos_identity"),
                     "–" if nat else f"{r['pos_excess']:+.4f}",
                     g("edit_identity"), g("aa_identity"),
                     rr("rho_preservation"), rr("rho_recovery")])
    ...
    tbl = axT.table(cellText=rows, colLabels=head, cellColours=colours,
                    colWidths=[0.31] + [0.099] * (len(head) - 1), loc="center", cellLoc="center")
    ...
    for (i, j), cell in tbl.get_celld().items():
        ...
        if i == 0:
            ...
            if j == 0:
                cell.set_text_props(ha="left", fontsize=6.2, color=acs.apc.white,
                                    fontweight="bold")
            if j == 3:                      # the excess column is what carries the argument
                cell.set_text_props(fontweight="bold")
    axT.set_title(f"Per-control identity to source and the ρ it produces  —  {panel}, "
                  f"blocks.{layer}", fontsize=9, fontweight="bold", loc="left")
    note = ("excess = identity to source − self-pair null (the identity between two independent "
    ...
    axT.text(0.0, -0.02, note, transform=axT.transAxes, fontsize=5.9, va="top",
             color=acs.ANNOTATION)

    sub = gs[1].subgridspec(1, 2, width_ratios=[3, 2], wspace=0.06)
    lo = fig.add_subplot(sub[0])
    hi = fig.add_subplot(sub[1], sharey=lo)
    _scatter_rho_vs_identity(lo, hi, panel, idm, rl, stats, conds, layer, "rho_preservation",
...
def figure_within_rung(panel: str, per_seq: pd.DataFrame, layer: int, out_dir: Path) -> None:
    ...
    for ax, c in zip(axes, conds):
        g = m[m["condition"] == c]
        ax.scatter(g["pos_identity"], g["rho_geodesic_vs_natural"], s=12,
                   color=COLOUR.get(c, acs.CONTROL_FALLBACK), edgecolor=acs.apc.white, linewidth=0.3)
    ...
    fig.suptitle(f"Within each rung, across families: does more retained identity mean higher ρ?  "
                 f"[{panel}, blocks.{layer}]", fontsize=8)
...
def write_report(panel: str, ident: pd.DataFrame, rho: pd.DataFrame, stats: pd.DataFrame,
                 within: pd.DataFrame, layer: int, out_dir: Path) -> None:
    spec = PANELS[panel]
    idm = ident.set_index("condition")
    rl = rho[(rho.panel == panel) & (rho.layer == layer)].set_index("condition")
    conds = [c for c in LADDER if c in idm.index]
    L = [f"# Nucleotide identity of each composition control to its source — {panel}",
         "",
         f"Run: `{spec['run']()}`  ·  headline layer: `blocks.{layer}`  ·  "
         f"recovery ground truth: {spec['recovery_name']}",
         f"Control sequences: "
         f"{'the embedded FASTAs on disk (exact)' if spec['exact_sequences'] else 'fresh draws from the same generators (the embedded draw is PYTHONHASHSEED-salted and cannot be regenerated; see draw_control)'}",
         "",
    ...
    for c in conds:
        r = idm.loc[c]
        g = lambda k: (f"{r[k]:.3f}" if k in r and np.isfinite(r[k]) else "–")  # noqa: E731
        pc = lambda k: (f"{100 * r[k]:.1f}%" if k in r and np.isfinite(r[k]) else "–")  # noqa: E731
        L.append(f"| {LABEL.get(c, c)} | {int(r['n_seqs'])} | {r['len_src']:.0f} | "
    ...
    for c in conds + ["natural"]:
        ...
        L.append(f"| {LABEL.get(c, c)} | {pi} | {ex} | "
                 f"{rl.loc[c, 'rho_preservation']:.3f} | {rl.loc[c, 'rho_recovery']:.3f} |")
    L += ["", "## Monotonicity tests at this layer", "",
    ...
    for _, s in stats[(stats.panel == panel) & (stats.layer == layer)].iterrows():
        ...
        L.append(f"| {s.rho_col} | {s.identity_metric} | {s.spearman_all_rungs:+.3f} | "
    ...
    if len(within):
        L += ["", "## Within each rung, across families", "",
              "| rung | ρ column | families | Spearman(family identity, family ρ) | p |",
              "|---|---|---|---|---|"]
        for _, s in within.iterrows():
            L.append(f"| {LABEL.get(s.condition, s.condition)} | {s.rho_col} | {int(s.n_families)} "
...
def main() -> None:
    ...
    ap.add_argument("--stage", choices=["identity", "figure", "all"], default="all",
    ...
    for panel in args.panel:
        ...
        cols = ["condition", "n_seqs", "pos_identity", "null_pos_identity", "pos_excess",
                "edit_identity", "aa_identity"]
        ...
        print("\n" + key[["rho_col", "spearman_all_rungs", "spearman_nucleotide_rungs",
                          "identity_range_nucleotide_rungs", "rho_range_nucleotide_rungs",
                          "recode_rho_residual"]].round(4).to_string(index=False))
    print("\nDone.")

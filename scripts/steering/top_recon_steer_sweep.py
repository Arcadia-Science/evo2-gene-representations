...
def score(gen: str, target_cont: str, priv_in_window: dict[int, str]) -> dict:
    ...
    matches = sum(1 for x, y in zip(g_aln, t_aln) if x == y and x != "-")
    # protein identity to target
    pg, pt = protein(gen), protein(target_cont)
    pa = nt_aligner().align(pg, pt)[0] if pg and pt else None
    aa_id = (sum(1 for x, y in zip(str(pa[0]), str(pa[1])) if x == y and x != "-") / max(len(str(pa[0])), 1)
             if pa is not None else np.nan)
...
def main() -> None:
    ...
    ap.add_argument("--layers", type=int, nargs="+", default=None, help="block indices; default all 32")
    args = ap.parse_args()
    OUT = Path(args.out)
    GEN = args.gen_bp
    ...
    div = pd.read_csv(DIV)
    targets = dict(zip(div["gene"], div["most_private_species"]))
    nsp = dict(zip(div["gene"], div["n_species"]))
    ...
    if scores_path.exists():
        ...
        done = set(zip(prev["gene"], prev["layer"]))
    rows: list[dict] = []
    ...
    for gene in genes:
        ...
        if n_tokens < 60:
            continue
        # target private coords (full CDS), then restrict to the generated window and re-index to cont
        priv_full = target_private_coords(cds, target)
        priv_win = {p - PROMPT_BP: b for p, b in priv_full.items() if PROMPT_BP <= p < PROMPT_BP + n_tokens}
        print(f"\n=== {gene} -> {target} | prompt {PROMPT_BP}bp, gen {n_tokens}bp | "
              f"{len(priv_win)} private bp in window | {len(cds)} species ===", flush=True)
        refs.write(f">{gene}|HUMAN_cont\n{human[PROMPT_BP:PROMPT_BP + n_tokens]}\n"
        ...
        for cond_name, L in conditions:
            ...
            try:
                if L is None:
                    gen_out = model.generate(prompt_seqs=[prompt] * args.n_samples, n_tokens=n_tokens,
                ...
                for k, cont in enumerate(gen_out.sequences):
                    ...
                    rows.append({"gene": gene, "target": target, "layer": cond_name, "sample": k,
                                 "n_species": len(cds), **sc})
                    gf.write(json.dumps({"gene": gene, "target": target, "layer": cond_name,
                                         "sample": k, "seq": cont}) + "\n")
                gf.flush()
            ...
            print(f"  {cond_name:9s} priv={g['pct_private_correct'].mean():5.1f}% "
                  f"aa={g['aa_id_to_target'].mean():5.1f}% indel={g['indel_bp'].mean():5.1f} "
                  f"stop={g['premature_stop'].mean():.2f}", flush=True)

    df = pd.DataFrame(rows)
    df.to_csv(scores_path, index=False)
    flog.close(); gf.close(); refs.close()

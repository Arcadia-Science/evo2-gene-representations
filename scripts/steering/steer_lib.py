...
GENE_FAMILY = {"HBE1": "globins", "HBZ": "globins", "HBQ1": "globins", "HBB": "globins",
               "CYP27B1": "cytochrome_p450", "HOXB1": "hox"}


# --------------------------------------------------------------------------- data
def load_cds(gene: str) -> dict[str, str]:
    """{species: uppercase CDS} for one gene, scanning its family fasta (header group|species|gene_id)."""
...
def steering(model, layer: str, steer_vec: np.ndarray | None, alpha: float, device: str = "cuda"):
    ...
    if steer_vec is not None and alpha != 0.0:
        vec = torch.as_tensor(np.asarray(steer_vec) * float(alpha), dtype=torch.float32, device=device)
...
def steering_field(model, layer: str, field: np.ndarray | None, alpha: float, device: str = "cuda"):
    """Add a DIFFERENT (position-matched) steering vector at each position of `layer`, scaled by alpha."""
...
def forward(model, seq: str, layer: str, steer_vec: np.ndarray | None = None,
            alpha: float = 0.0, device: str = "cuda"):
    ...
    try:
        ...
        if handle is not None:
            handle.remove()
    # Evo2's wrapper hands back StripedHyena.forward's return verbatim, which is a (logits, ...) tuple
...
@contextmanager
def overwriting(model, layer_vecs: dict[str, np.ndarray], alpha: float, keep_h: bool,
                preserve_norm: bool, device: str = "cuda"):
    """Replace-or-interpolate the residual stream at each block in `layer_vecs`. See _overwrite_hook."""
    handles = []
    try:
        for layer, vec in layer_vecs.items():
            if vec is None:
                continue
            v = torch.as_tensor(np.asarray(vec), dtype=torch.float32, device=device)
            handles.append(model.model.get_submodule(layer).register_forward_hook(
...
def forward_multi(model, seq: str, layer_vecs: dict[str, np.ndarray], device: str = "cuda") -> np.ndarray:
...
# --------------------------------------------------------------------------- steering vectors
def build_pooled_embeddings(model, cds: dict[str, str], layer: str, pool_mode: str = "mean",
                            device: str = "cuda") -> dict[str, np.ndarray]:
    """{species: pooled (H,) residual-stream vector at `layer`} for every ortholog of one gene."""
    return {sp: pool(forward(model, seq, layer, device=device)[1], pool_mode)
...
def steering_vectors(embeds: dict[str, np.ndarray]) -> dict[str, dict[str, np.ndarray]]:
    ...
    h = embeds[HUMAN]
    return {sp: {"h2t": embeds[sp] - h, "centroid": embeds[sp] - centroid}
            for sp in embeds if sp != HUMAN}
...
def codon_diagnostics(human_cds: str, target_cds: str):
    """Codon-align human vs target CDS (translate -> BLOSUM62 global protein align -> matched-codon blocks)."""
    blocks_h, blocks_t, ph, pt = codon_blocks(human_cds, target_cds)

    diagnostic, conserved = [], []
    for (hs, he), (ts, te) in zip(blocks_h, blocks_t):
        for k in range(he - hs):
            ...
            for c in range(3):
                ...
                h_idx = 3 * hi + c
                if hb != tb:
                    diagnostic.append({"h_idx": h_idx, "h_base": hb, "t_base": tb,
...
def readout(logits_s: np.ndarray, logits_b: np.ndarray, human_cds: str, diagnostic, conserved) -> dict:
    """Self-controlled steering metrics from steered vs baseline logits at diagnostic/conserved sites."""
    ...
    for s in diagnostic:
        ...
        tgt_gain.append(tg); hum_gain.append(float(ls[hid] - lb[hid])); dis_gain.append(dg)
        spec.append(tg - dg)
    hb_ids = [BASE_ID.get(b, -1) for b in human_cds]
    cons = [float(_logprobs_at(logits_s, j)[hb_ids[j]]) for j in conserved if j > 0 and hb_ids[j] >= 0]
    full = [float(_logprobs_at(logits_s, j)[hb_ids[j]]) for j in range(1, len(human_cds)) if hb_ids[j] >= 0]
    m = lambda x: float(np.mean(x)) if len(x) else np.nan
    return {"n_diag": len(dlo), "spec_effect": m(spec), "mean_dlogodds": m(dlo), "recovery": m(rec),
            "target_gain": m(tgt_gain), "human_drop": m(hum_gain), "distractor_gain": m(dis_gain),
            ...
            "dlo_aa": m(aa), "dlo_syn": m(syn), "conserved_logp": m(cons), "seq_logp": m(full)}


def _valid(diagnostic):
    """Diagnostic sites that contribute to readout (h_idx>0), in order — for aligning per-site lists."""
...
def build_field(model, human_cds: str, target_cds: str, layer: str, device: str = "cuda") -> np.ndarray:

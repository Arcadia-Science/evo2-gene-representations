...
def main() -> None:
    ...
    ap.add_argument("--layers", nargs="*", type=int, default=[24, 25, 26, 27],
    ...
    for li in args.layers:
        ...
        for i in range(n):
            per_gene.append({
                "layer": li, "gene": genes[i], "family": fams[i], "superfamily": supers[i],
                "has_opossum": bool(has_opossum[i]),
                "loo_cos": float(loo_cos[i]), "q_loo": float(q_loo[i]),
                "delta_norm": float(Dn[i]), "v_loo_norm": float(Vn[i]),
                "alpha_to_match_own_shift": float(q_loo[i] / Vn[i]),
                "frac_on_cone": float(frac_cone[i]), "abs_cos_gc": float(abs_gc[i]),
            })

        per_layer.append({
            "representation": args.representation, "layer": li,
    ...
    np.savez_compressed(args.out_dir / "loo_vectors.npz",
                        genes=np.array(genes), families=fams,
                        representation=np.array([args.representation]),
                        layers=np.array(args.layers), **vecs)

    pd.set_option("display.width", 240)
    print(f"representation={args.representation}  N={n}  candidate layers={args.layers}\n")
    print(pl[["layer", "v_norm", "rel_norm_v_over_pooled", "loo_cos_median", "loo_frac_pos",
              "q_loo_median", "q_loo_frac_pos", "alpha_median", "alpha_p25", "alpha_p75",
              "frac_on_cone_median", "abs_cos_gc_median", "loo_vector_agreement"]]
    ...
    print(pg.pivot_table(index="family", columns="layer", values="loo_cos", aggfunc="median")
          .to_string(float_format=lambda z: f"{z:+.3f}"))

    (args.out_dir / "stage3_config.json").write_text(json.dumps({
        "representation": args.representation, "layers": args.layers, "n_genes": n,
        "design": "leave-one-gene-out: gene i is steered by v_-i = mean_{j!=i} D_j",
        "alpha_convention": "alpha=1 injects v_-i; alpha_median reproduces the median gene's own "

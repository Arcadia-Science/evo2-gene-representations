...
def per_gene_gain(d: pd.DataFrame, cond: str, phi: float, n_col: str) -> pd.DataFrame:
    ...
    g = d.groupby(["gene", "condition"]).agg(
        pct=("metric", "mean"),
        n_sites=(n_col, "first"),
        n_samp=("sample", "count")).reset_index()
    ...
    for gene in ix:
        ...
        rows.append({
            ...
            "logit_gain": np.log(out["p_s"] / (1 - out["p_s"])) - np.log(out["p_u"] / (1 - out["p_u"])),
            "var": out["v_s"] + out["v_u"],
            "n_trials": out["n_s"],
        })
...
def wls_slope(x: np.ndarray, y: np.ndarray, w: np.ndarray) -> dict:
    ...
    nan = {"n": int(len(x)), "b0": np.nan, "b1": np.nan, "se1": np.nan, "p1": np.nan,
           "b2": np.nan, "se2": np.nan, "p2": np.nan}
    ...
    return {"n": int(len(x)), "b0": float(beta[0]),
            "b1": float(beta[1]), "se1": se1,
            "p1": float(2 * stats.norm.sf(abs(beta[1] / se1))) if se1 > 0 else np.nan,
            "b2": float(beta[2]), "se2": se2,
...
def mid_vs_extreme(gain: pd.DataFrame) -> dict:
    ...
    return {"n_mid": int(len(mid)), "n_ext": int(len(ext)),
            ...
            "wtd_mid_pp": round(wm, 3), "wtd_ext_pp": round(we, 3),
...
def main() -> None:
    ...
    if dnds_path.exists():
        ...
        dn = dn[dn.status == "ok"]
        keep = [c for c in DNDS_PRIMARY + DNDS_NEGCTRL
                if c in dn.columns and dn[c].notna().sum() >= 20]
    ...
    for arm in args.arms:
        ...
        try:
            d, spec = load_scores(args.run, arm, scores=args.scores, metric=args.metric,
        ...
        print("=" * 96)
        print(f"{arm}   overdispersion phi = {phi:.2f}  "
              f"(nominal trials divided by this)")
        print("=" * 96)
        for cond in CONDS:
            ...
            for pred in preds + ["perc_id_hp"]:
                ...
                role = ("primary" if pred in PRIMARY else
                        "legacy" if pred in LEGACY else
                        "dnds_primary" if pred in dnds_primary else
                        "dnds_negative_control" if pred in dnds_negctrl else "frame")
                rows.append({
                    "arm": arm, "condition": cond, "predictor": pred, "role": role,
                    "n": int(np.isfinite(x).sum()),
                    "wls_beta_per_sd": round(b1, 4), "wls_se": round(se1, 4), "wls_p": p1,
                    "wls_quad_per_sd2": round(fit["b2"], 4), "wls_quad_se": round(fit["se2"], 4),
                    "wls_quad_p": fit["p2"],
    ...
    for arm in res.arm.unique():
        for cond in CONDS:
            ...
            t = s[["predictor", "role", "n", "wls_beta_per_sd", "wls_se", "wls_p",
                   "spearman_rho", "spearman_p"]].copy()
    ...
    print(f"nominally p<0.05 (uncorrected): {len(nom)} of {len(tested)} "
          f"-- expected by chance {0.05 * len(tested):.1f}")
    if len(nom):
        print(nom.sort_values("wls_p")[["arm", "condition", "predictor", "role",
                                        "wls_beta_per_sd", "wls_p", "spearman_rho"]]
    ...
    if dnds_primary or dnds_negctrl:
        ...
        for role, pl in (("PREDICT ", dnds_primary), ("CONTROL ", dnds_negctrl)):
            for pred in pl:
                ...
                print(f"  {role}{pred:22s} best beta {best.wls_beta_per_sd:+.4f}/SD "
        ...
        for arm in conf.arm.unique():
            for cond in CONDS:
                ...
                for _, r in s.iterrows():
                    ...
                    print(f"  {flag} {r.predictor:22s} lin {r.wls_beta_per_sd:+.4f}/SD "
    ...
    if shape_rows:
        ...
        for _, r in sh.iterrows():
            print(f"    {r.arm.replace('stage4_', ''):28s} {r.condition:9s} "

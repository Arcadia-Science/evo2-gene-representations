...
def build_codon_alignment(gene: str, run: Path, wd: Path) -> tuple[list[str], int]:
    ...
    cmd = [str(TRIMAL), "-in", str(tdir / "aln_mafft.fasta"), "-backtrans", str(cds),
           "-gappyout", "-ignorestopcodon", "-fasta", "-out", str(codon_fa)]
    ...
    for taxon in (HUMAN, PLATYPUS):
        s = codon[taxon]
        back = "".join("-" if s[i:i + 3] == "---" else str(Seq(s[i:i + 3]).translate())
...
def run_codeml(wd: Path, model: int, tree: str, tag: str, timeout: int) -> Path:
    ...
    r = subprocess.run([str(CODEML), "codeml.ctl"], cwd=sub, capture_output=True, text=True,
                       timeout=timeout, stdin=subprocess.DEVNULL)
    mlc = sub / f"{tag}.mlc"
    if r.returncode != 0:
        raise RuntimeError(f"codeml {tag} exited {r.returncode}: "
...
def run_yn00(wd: Path, timeout: int) -> dict:
    ...
    r = subprocess.run([str(YN00)], cwd=sub, capture_output=True, text=True, timeout=timeout,
    ...
    for line in out.read_text().splitlines():
        ...
        if m:
            g = m.groups()
            pairs.append({"a": int(g[0]), "b": int(g[1]), "omega": float(g[6]),
                          "dN": float(g[7]), "dS": float(g[9])})
    if not pairs:
    ...
    if focal:
        rec.update({"dN_hp_yn": focal[0]["dN"], "dS_hp_yn": focal[0]["dS"],
    ...
    if bg:
        ...
        rec["omega_background_yn"] = st.median(p["omega"] for p in bg
                                              if 0 <= p["omega"] < 10) if bg else float("nan")
...
def parse_mlc(mlc: Path) -> dict:
    ...
    for key, pat in (("tree_len", r"^tree length =\s*([\d.]+)"),
    ...
    for line in text.splitlines():
        m = re.match(r"^\s*(\d+)\.\.(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+"
                     r"([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s*$", line)
        if m:
            g = m.groups()
            rows.append({"parent": int(g[0]), "child": int(g[1]), "t": float(g[2]),
                         "N": float(g[3]), "S": float(g[4]), "omega": float(g[5]),
                         "dN": float(g[6]), "dS": float(g[7])})
    out["branches"] = rows
    return out
...
def do_gene(gene: str, run: Path, models: list[str], timeout: int, max_codons_fr: int,
            min_codons_per_branch: float) -> dict:
    wd = run / "stage5" / "dnds" / gene
    wd.mkdir(parents=True, exist_ok=True)
    rec: dict = {"gene": gene, "status": "ok", "fr_status": "not_requested",
    ...
    try:
        ...
        if "m0" in models:
            ...
            rec.update({"lnL_m0": p.get("lnL"), "np_m0": p.get("np"),
                        "omega_m0": p.get("omega"), "kappa_m0": p.get("kappa"),
                        "tree_len_m0": p.get("tree_len"),
                        "tree_dN_m0": p.get("tree_dN"), "tree_dS_m0": p.get("tree_dS")})
        ...
        if "m2" in models:
            ...
            rec.update({"lnL_m2": p.get("lnL"), "np_m2": p.get("np"),
                        "tree_dN_m2": p.get("tree_dN"), "tree_dS_m2": p.get("tree_dS")})
        ...
        if "fr" in models:
            ...
            if n_codons > max_codons_fr:
                ...
                rec["fr_status"] = (f"skipped_unidentifiable({per_branch:.1f} codons/branch < "
                                    f"{min_codons_per_branch})")
            else:
                try:
                    p = parse_mlc(run_codeml(wd, 1, "tree_plain.nwk", "fr", timeout))
                    rec.update({"lnL_fr": p.get("lnL"), "np_fr": p.get("np"),
                                "tree_dN_fr": p.get("tree_dN"), "tree_dS_fr": p.get("tree_dS")})
...
def main() -> None:
    ...
    ap.add_argument("--models", nargs="+", default=["yn", "m0", "m2", "fr"],
                    choices=["yn", "m0", "m2", "fr"])
    ap.add_argument("--timeout", type=int, default=3600, help="per codeml model, seconds")
    ap.add_argument("--max-codons-fr", type=int, default=3000,
    ...
    ap.add_argument("--min-codons-per-branch-fr", type=float, default=20.0,
    ...
    log(f"{len(genes)} genes, models={args.models}, jobs={args.jobs}, "
        f"longest first ({order.get(genes[0], 0):.0f} aa)")

    recs, branches, t0 = [], [], time.time()
    with ProcessPoolExecutor(max_workers=args.jobs) as ex:
        futs = {ex.submit(do_gene, g, args.run, args.models, args.timeout,
                          args.max_codons_fr, args.min_codons_per_branch_fr): g for g in genes}
        for n, fut in enumerate(as_completed(futs), 1):
            res = fut.result()
            recs.append(res["rec"])
            branches += res["branches"]
            r = res["rec"]
            log(f"[{n}/{len(genes)}] {r['gene']} {r['status']} yn={r['yn_status']} "
    ...
    if len(ok):
        ...
        if "m2_converged" in ok.columns:
            ...
            log(f"m2 non-convergent (lnL_m2 < lnL_m0, omega estimates untrustworthy): "
                f"{bad_conv}/{len(ok)}")
        ...
        if "dS_saturated" in ok.columns:
            log(f"  platypus dS > {DS_SATURATION} in {int(ok.dS_saturated.sum())}/{len(ok)}"
                " genes -- use BACKGROUND dS as the negative control")
    bad = df[df.status != "ok"]
    if len(bad):
        log(f"FAILED {len(bad)}: {dict(bad.status.value_counts())}")

    (stage5 / "stage5d_config.json").write_text(json.dumps(
        {"models": args.models, "method": "1 (one branch at a time)",
         "CodonFreq": "2 (F3x4)", "cleandata": 0, "icode": 0,
         ...
         "topology": "stage5b topology.nwk (VertLife MamPhy pruned per gene, lengths stripped)",
         "tip_order": "human=1, platypus=2 (forced), background = all non-focal branches",
         "dS_saturation_threshold": DS_SATURATION,
         "max_codons_fr": args.max_codons_fr, "timeout_s": args.timeout,

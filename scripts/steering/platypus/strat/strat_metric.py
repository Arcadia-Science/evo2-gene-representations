...
METRICS: dict[str, dict[str, str]] = {
    ...
    "legacy": {
        ...
        "gloss": ("PRE-FIX pairwise human-vs-platypus difference, scored through the protein "
...
RESCORE_CMD = ("uv run python scripts/steering/platypus/strat/stage4_rescore_nt.py \\\n"
               "      --run {run} --dir {arm_dir}")
...
def load_scores(run: Path, arm_dir: str, *, scores: str | None = None,
                metric: str = DEFAULT_METRIC, min_voters: int = 1,
    ...
    if not quiet:
        if metric == "legacy":
            print("[metric] *** LEGACY COLUMN -- historical reproduction only. This is the "
        ...
        if voters is not None:
            print(f"[metric] voting species per gene: median {report['voters_median']:.0f}, "
        ...
        if unscorable:
            ...
            print(f"[metric] WARNING {len(unscorable)}/{report['n_genes']} genes have NO "

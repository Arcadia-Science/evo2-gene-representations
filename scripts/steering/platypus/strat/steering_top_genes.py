...
def short_of(mspec: dict) -> dict:
    return {
        SITE_KEY: mspec["short"],
        "aa_id_to_target": "aa identity\n(pp)",
        "indel_frac": "indel burden\n(pp)",
        "stop_density": "stops/100\ncodons (pp)",
    }
...
def criteria_of(mspec: dict) -> list:
    ...
    return [
        ...
        (
            ...
            "coherence_level",
            "level",
            "most coherent sequences after steering — fewest indels and premature "
            "stops in absolute "
...
def main() -> None:
    ...
    se = per_gene_se(
        run, args.arm_dir, args.condition, mspec, scores=args.scores, min_voters=args.min_voters
    ).pivot(index="gene", columns="metric", values="se")
    ...
    for ax, (title, rank_key, basis, gloss) in zip(axes, CRITERIA, strict=False):
        ...
        for i, g in enumerate(genes):
            for j, k in enumerate(keys):
                ...
                ax.text(
                    j,
                    i - 0.14,
                    f"{num(base.loc[g, k])} → {num(steer.loc[g, k])}",
                    ha="center",
                    va="center",
                    fontsize=6.3,
                    color=col,
            ...
            rows.append(
                {
                    ...
                    "ranked_on": basis,
                    **{f"unsteered_{cname(k)}": round(float(base.loc[g, k]), 3) for k in keys},
                    **{f"steered_{cname(k)}": round(float(steer.loc[g, k]), 3) for k in keys},
    ...
    cb = fig.colorbar(
        sm, ax=axes, orientation="horizontal", fraction=0.045, pad=0.10, aspect=55, ticks=[-1, 0, 1]
    )
    cb.ax.set_xticklabels(["worse", "unsteered", "better"], fontsize=7)
    cb.set_label(
        "cell fill = per-readout Δ, sign-corrected so ink = better, normalised by that "
    ...
    fig.suptitle(
        f"Which genes steer best, and what it costs them elsewhere  "
    ...
    show = [
        ...
        "symbol",
        "perc_id_hp",
        f"steered_{mspec['col']}",
        "steered_aa_id_to_target",
        "steered_indel_frac",
        "steered_stop_density",
        "unsteered_indel_frac",
        "unsteered_stop_density",
    ]

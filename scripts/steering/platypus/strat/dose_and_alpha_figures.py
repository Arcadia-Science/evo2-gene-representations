...
def per_gene_delta(
    d: pd.DataFrame, cond: str, col: str, genes: set[str] | None = None
) -> pd.Series:
...
def main() -> None:
    ...
    present = sorted({float(m.group(1)) for c in d.condition.unique() if (m := dose_re.match(c))})
    ...
    fig, (ax, ax_t) = plt.subplots(
        1, 2, figsize=(8.8, 4.4), dpi=300, gridspec_kw={"width_ratios": [1, 0.46], "wspace": 0.05}
    )
    # Compare each additive arm with the same gene's unsteered cell.
    series = [
        (spec["delta_label"], "metric", INK, "o"),
    ...
    if partial:
        print(
            f"  SKIPPING incomplete conditions (< {args.min_frac:.0%} of {n_ref} genes): "
    ...
    if any(counts[c] < n_ref for c in plotted):
        ...
        print(
            f"  PROVISIONAL: {', '.join(incomplete)} still running; all series restricted to "
            ...
            rows.append(
                {
                    "series": label,
                    "alpha": a,
                    "mean_delta_pp": g.mean(),
                    "n_genes": len(g),
                    "provisional": common is not None,
                }
            )
        means[col] = ys
    ...
    ax.set_title(
        f"Dose–response and its protein cost — {args.metric} sites", fontsize=10, fontweight="bold"
    )
    ...
    cells, rate_col = [], []
    for a, p, c in zip(alphas, priv, aa, strict=False):
    ...
    ax_t.text(
        0.5,
        0.98,
        "Δ vs the same gene's unsteered cell",
    ...
    if common is not None:
        ax_t.text(
            0.5,
            -0.06,
            f"PROVISIONAL · run in progress\nsame {len(common)}/{n_ref} genes throughout",
            transform=ax_t.transAxes,
            ha="center",
            va="top",
            fontsize=7,
    ...
    if pub.is_on():
        ...
        fig, axgrid = plt.subplots(
            nrow, ncol, dpi=300, sharey=True, figsize=pub.size(pub.FULL, PUB_PANEL_H * nrow)
        )
        axes = axgrid.ravel()
        ...
        fig, axes = plt.subplots(
            1, len(keys), figsize=(3.05 * len(keys), 3.7), dpi=300, sharey=True
        )
    srows = []
    for ax_s, s in zip(np.atleast_1d(axes), keys, strict=False):
        ...
        if n_ok != len(genes):
            print(
                f"  WARNING stratum {s}: only {n_ok} of {len(genes)} genes scorable on this "
        ...
        ax_s.set_title(
            (
                ...
                if pub.is_on()
                else f"stratum {s} · {sub.perc_id_hp.min():.0f}–{sub.perc_id_hp.max():.0f}% id"
            ),
            fontsize=9,
            fontweight="bold",
        )
    ...
    if pub.is_on():
        ...
        if common is not None:
            fig.text(
                0.5,
                -0.02,
                f"PROVISIONAL · run in progress · same {len(common)}/{n_ref} genes throughout",
                ha="center",
                fontsize=7.5,
                fontweight="bold",
                color=WARM,
    ...
    for ax_s, s in zip(np.atleast_1d(axes), keys, strict=False):
        ...
        for label, col, colr, mk in series:
            ...
            for a, c, y in zip(xs0, conds0, ys, strict=False):
                lrows.append(
                    {
                        "stratum": s,
                        "alpha": a,
        ...
        n_lab = (
            f"n = {n_ok} genes"
            if n_ok == len(genes)
            else f"n = {n_ok} of {len(genes)} genes scorable"
        )
        ax_s.set_title(
    ...
    fig.suptitle(
        "Absolute levels by conservation stratum "
    ...
    aow = (
        d[d.condition == "add_own"]
        .assign(a=lambda x: x.alpha_own.map(_alpha))
        .groupby("gene")["a"]
        .mean()
        .rename("alpha_own")
    )
    ...
    ax.set_ylabel("genes")
    ax.set_title(
        f"Own-scaled dose per gene\nmedian {m.alpha_own.median():.2f}, "
        f"{100 * (m.alpha_own < 1).mean():.0f}% below α=1",
        fontsize=9,
    )
    ...
    ax.set_title(
        f"Per gene: own-scaled vs fixed\nmean Δ {m['diff'].mean():+.2f} pp, "
        f"own wins in {100 * (m['diff'] > 0).mean():.0f}% of genes",
        fontsize=9,
    )
    ...
    fig.suptitle(
        "Two dose conventions: identical intervention (α=1) vs identical effect size "
        "(α_i matched to the gene's own shift)",
        fontsize=11,
        fontweight="bold",
    )
    fig.tight_layout()
    ...
    print(f"  genes                      {len(m)}")
    print(
        f"  alpha_own  median {m.alpha_own.median():.3f}  IQR "
        f"{m.alpha_own.quantile(0.25):.3f}-{m.alpha_own.quantile(0.75):.3f}  "
        f"range {m.alpha_own.min():.3f}-{m.alpha_own.max():.3f}"
    )
    print(f"  Δ fixed α=1                {m.fixed.mean():+.3f} pp")
    print(f"  Δ own-scaled               {m.own.mean():+.3f} pp")
    print(
        f"  paired difference          {m['diff'].mean():+.3f} pp "

...
def tier_c_gate(D: np.ndarray, clusters: np.ndarray, rng: np.random.Generator) -> pd.DataFrame:
    ...
    for k in K_GRID:
        ...
        for _ in range(N_DRAW):
            ...
            same.append(
                panel_pair_cos(
                    D, rng.choice(own, k, replace=False), rng.choice(oth, k, replace=False)
                )
            )
            ...
            rows.append(
                {
                    "k": k,
                    "n_draws": len(same),
                    "cos_same_vs_other": np.nan,
                    "cos_random_vs_random": np.nan,
                    "contrast": np.nan,
                    "usable": False,
                }
            )
        ...
        rows.append(
            {
                "k": k,
                "n_draws": len(same),
                "cos_same_vs_other": round(s, 4),
                "cos_random_vs_random": round(r, 4),
...
def main() -> None:
    ...
    if cpath.exists():
        ...
        print(
            f"TIER C -- cluster-matched vs cross-cluster panel directions "
        ...
        summary["tier_c"] = {
            "best_contrast_at_feasible_k": None if not np.isfinite(best) else round(float(best), 4),
    ...
    if pgp.exists():
        ...
        print(
            f"  {td['n_genes']} genes, {td['n_distinct_layers']} distinct peak layers, "
        ...
        print(
            f"  unrestricted (DIAGNOSTIC ONLY, do not gate on this): "

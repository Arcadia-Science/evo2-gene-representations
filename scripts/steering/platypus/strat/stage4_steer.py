...
def main() -> None:
    ...
    ap.add_argument("--layers", nargs="+", default=["blocks.27"],
    ...
    ap.add_argument("--no-split-alpha", action="store_true",
    ...
    ap.add_argument("--panel-k", type=int, default=5,
    ...
    ap.add_argument("--per-gene-layers", type=Path, default=None,
    ...
    ap.add_argument("--cond-suffix", default="",
    ...
    layer_label = (args.layers[0] if len(lis) == 1
                   else "blocks." + "+".join(str(i) for i in lis))
    out = args.out or (args.run / f"stage4_{args.representation}_"
                       f"{layer_label.replace('.', '').replace('+', '_')}")
    ...
    # over genes rather than --layers. Everything downstream keys off `lis_of(gene)`, and single-layer
    ...
    if "gc_removed" in args.arms and not GCAX:
        raise SystemExit("gc_removed needs gc_axis_L* in loo_vectors.npz -- re-run stage3_select.py")
    ...
    if n_shift:
        log(f"shifted-window panel: {n_shift}/{len(genes)} genes have a non-zero prompt offset "
    ...
    for g in genes:
        ...
        while n_diag < args.min_sites and n_tokens < min(len(cont), args.max_gen_bp):
            ...
            n_diag = sum(1 for i in diag_all if i < n_tokens)
        ns = int(np.clip(math.ceil(args.trial_target / max(n_diag, 1)),
                         args.min_samples, args.max_samples))
        plan[g] = {"n_tokens": n_tokens, "n_diag": n_diag, "n_samples": ns,
                   "off_h": off_h.get(g, 0), "off_p": off_p.get(g, 0),
                   "diag": {i: b for i, b in diag_all.items() if i < n_tokens},
                   "cont": cont[:n_tokens]}
        if len(ch[g][off_h.get(g, 0):off_h.get(g, 0) + PREFIX_BP]) != PREFIX_BP:
            raise AssertionError(f"{g}: prompt shorter than {PREFIX_BP} bp at offset "
    ...
    pd.DataFrame([{"gene": g, **{k: v for k, v in plan[g].items() if k not in ("diag", "cont")},
                   "stratum": int(prow[g].stratum), "usable": g in usable} for g in genes]
                 ).to_csv(out / "scoring_plan.csv", index=False)
    log(f"{len(usable)}/{len(genes)} genes usable; dropped {len(dropped)} for <{args.min_sites} "
        f"sites: {dropped[:10]}")
    ns_arr = np.array([plan[g]["n_samples"] for g in usable])
    log(f"n_samples: mean {ns_arr.mean():.1f} min {ns_arr.min()} max {ns_arr.max()}  |  "
    ...
    Dpool = np.load(args.run / "stage2" / "pooled_representations.npz",
                    allow_pickle=True)[args.representation]
    ...
    # depth (~1.4 / 2.4 / 5.3 at L25/26/27), so a 1/3-split band delivers less total residual-relative
    # perturbation than L27 alone. rel_norm per layer is logged below so the realised dose is on record.
    split = 1.0 if (args.no_split_alpha or len(lis) == 1) else 1.0 / len(lis)
    if len(lis) > 1:
        log(f"band injection at {len(lis)} layers, alpha split factor {split:.4f}"
    ...
    for gi, gene in enumerate(usable):
        ...
        def scaled(vecs: dict[int, np.ndarray], al: dict[int, float]) -> dict[str, np.ndarray]:
            """steering_multi takes ALREADY-SCALED vectors, so alpha is folded in here. For a single layer this is bit-identical to S.steering(model, layer, vec, alpha)."""
        ...
        if "cross_gene" in args.arms:
            ...
            cv = {li: V[li][j].astype(np.float64) / np.linalg.norm(V[li][j]) * vn[li] for li in g_lis}
        ...
        if "cone_removed" in args.arms:
            ...
            conditions.append(("add_cone_removed_a1.0", scaled(cv, {li: 1.0 for li in g_lis}), None))
        ...
        if "cluster_panel" in args.arms:
            ...
            if len(own) >= args.panel_k and len(oth) >= args.panel_k:
                for tag, pool in (("same", own), ("other", oth)):
                    ...
                    conditions.append((f"panel_{tag}_k{args.panel_k}",
                                       scaled(pv, {li: 1.0 for li in g_lis}), None))
            else:
                log(f"  {gene}: cluster too small for k={args.panel_k} "
        ...
        # which isolates the direction change from the ~2.8x norm collapse a bare replace also causes.
        ...
        log(f"[{gi + 1}/{len(usable)}] {gene} s{int(prow[gene].stratum)} "
        ...
        for cond, layer_vecs, op in conditions:
            ...
            if (gene, cond) in done:
                continue
            try:
                ctx = (S.steering_multi(model, layer_vecs) if op is None
                       else S.overwriting(model, layer_vecs, alpha=op[0], keep_h=op[1],
                                          preserve_norm=op[2]))
                with ctx:
                    o = model.generate(prompt_seqs=[prompt] * p["n_samples"],
                                       n_tokens=p["n_tokens"], temperature=args.temperature,
                                       top_k=args.top_k, verbose=0)
                for k, cont in enumerate(o.sequences):
                    sc = score(cont, p["cont"], p["diag"])
                    rows.append({"gene": gene, "stratum": int(prow[gene].stratum),
                                 ...
                                 "representation": args.representation, "condition": cond,
                                 "sample": k,
                                 ...
                                 "n_diag_scorable": p["n_diag"], "gen_bp": p["n_tokens"],
    ...
    (out / "stage4_config.json").write_text(json.dumps(
        {"layers": args.layers, "layer_label": layer_label, "n_layers": len(lis),
         "injection": "each layer gets its own v_-i, applied simultaneously (steering_multi)",
         "alpha_split_factor": split,
         "alpha_split_note": ("alpha divided by n_layers so the summed alpha matches a single-layer "
                              "run; equalises summed alpha, not summed residual-relative dose"),
         "representation": args.representation, "arms": args.arms,
         "alphas": args.alphas, "trial_target": args.trial_target, "gen_bp": args.gen_bp,
         "max_gen_bp": args.max_gen_bp, "min_sites": args.min_sites,
         "n_genes": len(usable), "dropped": dropped, "temperature": args.temperature,
         "top_k": args.top_k,
         "decoding": ("greedy (top_k=1 short-circuits to argmax; temperature inert)"
                      if args.top_k == 1 else
                      f"top-{args.top_k} truncation then temperature {args.temperature}"),

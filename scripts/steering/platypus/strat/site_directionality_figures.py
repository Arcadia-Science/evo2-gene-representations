...
GC_LABELS = {
    "gc_up": "GC-increasing\nA/T → G/C",
    "gc_down": "GC-decreasing\nG/C → A/T",
    "gc_neutral": "GC-neutral\nA↔T, G↔C",
}
# GC up/down is a signed contrast, so it takes a warm/cool pair around a neutral.
GC_COLOURS = {"gc_up": acs.apc.dragon, "gc_down": acs.apc.aegean, "gc_neutral": acs.SERIES_MUTED}
SET_LABEL = {
    "private": "private bp (platypus base carried by no other sampled mammal)",
...
def dose_labels(suffix: str) -> dict[str, str]:
    return dict(zip(dose_ladder(suffix), ["0", "0.5", "1", "2", "3", "4"], strict=False))
...
def arms_for_layer(summ: pd.DataFrame, layer: int) -> tuple[list[str], list[str]]:
    ...
    other = sorted(
        c
        for c in have
        if c not in set(dose_ladder(suffix))
        and (c.endswith("_L24") if suffix else not c.endswith("_L24"))
    )
    return ladder, other


def _thin(summ: pd.DataFrame, min_frac: float = 0.95) -> set[str]:
    """Conditions covering well under the full panel -- flagged in every label, never silently
    mixed.
    """
...
def label_for(
    cond: str, dl: dict[str, str], thin: set[str], n: int | None = None, strip_alpha: bool = True
) -> str:
...
def fig18(
    ...
    fig, ax = plt.subplots(figsize=(pub.size(pub.HALF, PUB_PANEL_H) if pub.is_on() else (6.1, 5.2)))
    ...
    if len(lad) > 1:
        ...
        for i in range(len(lad) - 1):
            ax.annotate(
                ...
                xytext=(x[i], y[i]),
                arrowprops=dict(
                    arrowstyle="->", color=acs.apc.dragon, lw=1.1, alpha=0.75, shrinkA=6, shrinkB=6
                ),
                zorder=2,
            )
    for c in lad:
        ...
        lab = (
            dl.get(c, label_for(c, dl, thin, int(s.loc[c, "n_genes_total"])))
            if pub.is_on()
            else label_for(c, dl, thin, int(s.loc[c, "n_genes_total"]))
        )
        ax.annotate(
            lab,
            (s.loc[c, "d_L"], s.loc[c, "d_C"]),
            textcoords="offset points",
            xytext=(0, 11),
            ha="center",
            fontsize=6.5,
            color=acs.apc.cinnabar,
            zorder=5,
        )
    ...
    ax.scatter(
        0, 0, s=42, color=acs.SERIES_MUTED, marker="o", edgecolor=acs.apc.white, lw=0.8, zorder=4
    )
    ...
    if not variant_arms:
        ...
        if skipped:
            print(
                f"  fig18: not plotting {len(skipped)} variant arm(s) "
    ...
    for c in others:
        ...
        ax.scatter(x, y, s=area(c), color=col, marker="s", edgecolor="white", lw=0.6, zorder=3)
        ...
        if not pub.is_on():
            ax.annotate(
                label_for(c, dl, thin, int(s.loc[c, "n_genes_total"]), strip_alpha=False),
                (x, y),
                textcoords="offset points",
                xytext=(-9, 0),
                ha="right",
                va="center",
                fontsize=5.8,
                color=acs.apc.charcoal if "random" in c else acs.apc.depths,
                zorder=5,
            )
    ...
    if pub.is_on():
        ...
        for e in EXT:
            fig.savefig(
                out / f"18_leave_human_vs_platypus_choice{sfx}.{e}", dpi=300, bbox_inches="tight"
            )
    plt.close(fig)

    # The CSV keeps EVERY arm even when the figure hides some -- the numbers are valid and
    # `add_gc_removed` in particular is the run's main mechanistic result. `plotted` records what
    # the
    ...
    tab = s.loc[
        keep, ["n_genes_total", "d_A_cov", "d_A_all", "d_L", "d_C", "d_K", "d_excess"]
    ].copy()
...
def fig19(
    gc: pd.DataFrame, summ: pd.DataFrame, out: Path, layer: int, site_set: str, sfx: str = ""
) -> pd.DataFrame:
    ...
    set_pub_style(title_size=9, tick_size=7)
    fig, axes = plt.subplots(1, 2, figsize=(10.2, 4.0), gridspec_kw={"width_ratios": [1.35, 1]})
    ax = axes[0]
    ...
    for i, cls in enumerate(GC_CLASSES):
        ...
        ax.bar(
            np.arange(len(conds)) + (i - 1) * w,
            d.d_A_mean,
            width=w,
            color=GC_COLOURS[cls],
            label=GC_LABELS[cls].split("\n")[0],
        )
    ax.axhline(0, color=acs.ZERO_LINE, lw=0.7)
    ...
    tab = (
        g[g.condition.isin(conds)]
        .pivot_table(
            index="condition",
            columns="gc_class",
            values=["d_A_mean", "d_A_frac_improved", "n_sites_per_gene"],
        )
        .reindex(conds)
    )
...
def fig20(
    summ: pd.DataFrame, per: pd.DataFrame, out: Path, layer: int, site_set: str, sfx: str = ""
) -> pd.DataFrame:
    ...
    set_pub_style(title_size=9, tick_size=7)
    fig, axes = plt.subplots(1, 2, figsize=(11.0, 4.2), gridspec_kw={"width_ratios": [1.15, 1]})
    ax = axes[0]
    x = np.arange(len(conds))
    ax.bar(x - 0.2, s.loc[conds, "d_A_cov"], width=0.4, color=acs.apc.cloud, label="raw Δ recovery")
    ax.bar(
        x + 0.2,
        s.loc[conds, "d_excess"],
        width=0.4,
        color=acs.apc.aster,
    ...
    ax.set_title(
        f"How much of the gain survives the composition null — blocks.{layer}\n"
    ...
    if data:
        ...
        ax.set_xticks(
            range(1, len(keep) + 1),
            [label_for(c, dl, thin) for c in keep],
            rotation=45,
            ha="right",
            fontsize=6,
        )
...
def main() -> None:
    ...
    for layer in args.layers:
        ...
        for site_set in sets:
            ...
            print(
                t20[["d_A_cov", "d_excess", "excess_share_of_gain"]].to_string(

"""Hypothesis figures for a conservation-stratified platypus run. Nine figures, each tied to a pre-registered hypothesis rather than to a pipeline stage:"""
...
from stage5_merge import CONFIRMATORY, CONFIRM_ROLE, loo_pc1_cos, resid  # noqa: E402
...
STRATA = ["s0\nfastest", "s1", "s2", "s3", "s4\nconserved"]
OUTCOME_LABEL = {"loo_cos": "orientation  cos(D_i, v_-i)",
...
def per_gene_table(run: Path, layer: int, mode: str, with_pc1: bool = True) -> pd.DataFrame:
    ...
    df = (pairs[["gene", "stratum", "perc_id_hp", "cds_len_human"]]
    ...
    if with_pc1:
        ...
        if cache.exists():
            ...
            pc1 = pd.DataFrame({"gene": [str(g) for g in npz["genes"]],
                                "loo_cos_pc1": loo_pc1_cos(D)})
...
def gain_table(s: pd.DataFrame, spec: dict) -> pd.DataFrame:
    ...
    g = (s.groupby(["gene", "condition"])
         .agg(p=("metric", "mean"),
              n_sites=(spec["n_col"], "mean"),
              n_samples=("n_samples", "mean")).reset_index())
...
def fig1_qc(run: Path, out: Path) -> None:
    ...
    for j, c in enumerate(tab.columns):
        axes[1].bar(tab.index, tab[c], 0.62, bottom=bottom, color=[INK, WARM, PLUM][j % 3],
                    edgecolor=acs.apc.white, lw=1.0, label=f"{c}  (n={int(tab[c].sum())})")
        bottom += tab[c].values
    ...
    fig.suptitle(f"Stage 1 QC — {summ['n_kept']} kept of {summ['n_examined']} examined, "
                 f"--max-start-codon {summ['max_start_codon']}", y=1.03)
    fig.tight_layout()
...
def _shape_panel(ax, x, y, strata, fit, xlab, ylab, title, rho=None):
    ...
    if fit is not None and np.isfinite(fit["b_linear"]):
        ...
        ax.plot(zz, y.mean() + fit["b_linear"] * zz + fit["b_quad"] * zz ** 2,
                color=WARM, lw=2.0, zorder=3, label="linear + quadratic")
...
def fig2_h1ab(run: Path, out: Path, df: pd.DataFrame, layer: int, mode: str) -> None:
    ...
    rhos = pd.read_csv(run / "stage5" / "rate_vs_direction.csv").set_index(
        ["predictor", "outcome"]).rho
    ...
    for r, outc in enumerate(outcomes):
        for c, pred in enumerate(CONFIRMATORY):
            ...
            if len(row):
                q = row.iloc[0]
                fit = {"b_linear": q.b_linear, "p_linear": q.p_linear, "b_quad": q.b_quad,
                       "p_quad": q.p_quad, "mid_minus_ext": q.mid_minus_ext,
                       "p_mid_ext": q.p_mid_ext}
            neg = "NEGATIVE CONTROL" in CONFIRM_ROLE[pred]
            _shape_panel(axes[r, c], df[pred].to_numpy(float), df[outc].to_numpy(float),
                         df.stratum.to_numpy(int), fit,
                         f"z({pred})" + ("   [neg. control]" if neg else ""),
                         OUTCOME_LABEL[outc] if c == 0 else "", pred,
                         rho=rhos.get((pred, outc), np.nan))
            if neg:
                for s in axes[r, c].spines.values():
                    s.set_color(WARM)
                    s.set_linewidth(1.4)
    handles = [Line2D([], [], color=WARM, lw=2.0,
               ...
               Line2D([], [], ls="", label="mid−ext is a property of the OUTCOME, so it repeats "
                                           "across the four predictors in a row")]
    fig.legend(handles=handles, loc="lower center", ncol=2, fontsize=8, frameon=False,
               bbox_to_anchor=(0.5, -0.04))
    fig.suptitle(f"H1a orientation / H1b magnitude vs the four confirmatory rate predictors — "
                 f"{mode}, block {layer}, n = {len(df)} genes, one point per gene", y=1.005)
    fig.tight_layout()
...
def fig3_h2a(run: Path, out: Path, df: pd.DataFrame, gains: dict) -> None:
    ...
    for r, cond in enumerate(("add_a1.0", "add_own")):
        ...
        for c, pred in enumerate(CONFIRMATORY):
            ...
            if len(row):
                q = row.iloc[0]
                zz = np.linspace(z.min(), z.max(), 100)
                ax.plot(zz, b0[cond] + q.wls_beta_per_sd * zz + q.wls_quad_per_sd2 * zz ** 2,
                        color=WARM, lw=2.0, zorder=3)
                ...
                ax.set_title(f"{pred}\nslope/SD {q.wls_beta_per_sd:+.4f} {pstr(q.wls_p)}   "
                             ...
                             f"Holm {q.p_holm:.3g}", fontsize=6.5)
    ...
    fig.suptitle("H2a — steering gain vs the same four confirmatory predictors "
...
def fig4_h1c(run: Path, out: Path) -> None:
    ...
    a = axes[0]
    a.fill_between(ex.k, ex.sil_null_mean - 2 * ex.sil_null_sd,
                   ex.sil_null_mean + 2 * ex.sil_null_sd, color=WARM, alpha=0.18, lw=0)
    ...
    a.scatter([ks], [ex.loc[ex.k == ks, "silhouette"].item()], s=90, facecolor="none",
    ...
    a.set_title(f"H1c part 1 existence — PASS\nexcess {summ['k_selected'] and ex.sil_excess.max():+.3f} "
                f"at k*={ks}, global p={summ['p_global']:.4f}", fontsize=8)
    ...
    b.text(2.45, 0.605, "a stable partition\nsits here", ha="right", va="bottom",
           fontsize=6, color=WARM)
    ...
    c.set_yticks(range(len(tab)), [f"c{i} (n={int(n)})" for i, n in
                                   zip(tab.index, tab.sum(axis=1))], fontsize=7)
    for i in range(tab.shape[0]):
        for j in range(tab.shape[1]):
            v = tab.values[i, j]
            c.text(j, i, str(v), ha="center", va="center", fontsize=7,
...
def fig5_matrix(run: Path, out: Path) -> None:
    ...
    BY_MEANING = [
        ...
        ("total divergence", ["tree_len", "diameter", "focal_target_dist", "mean_target_dist",
                              "background_rate"]),
    ...
    for _, q in d.iterrows():
        col = {"delta_norm": "||D_i||", "loo_cos": "loo_cos",
               "loo_cos_pc1": "loo_cos PC1-rm"}[q.outcome]
    ...
    fig, ax = plt.subplots(figsize=(pub.size(pub.FULL, PUB_MATRIX_H) if pub.is_on()
                                    else (7.6, 8.0)))
    ...
    for i in range(M.shape[0]):
        for j in range(M.shape[1]):
            if np.isfinite(M.values[i, j]):
                ax.text(j, i, f"{M.values[i, j]:+.2f}" + ("*" if sig.values[i, j] else ""),
    ...
    for start, n, title in groups:
        ...
        if not pub.is_on() or n > 1:
            ax.text(-0.42, start + n / 2 - 0.5, title, rotation=90, va="center", ha="center",
                    fontsize=6.6, color=GREY, transform=trans, clip_on=False)
        ax.plot([-0.335, -0.335], [start - 0.35, start + n - 0.65], color=acs.GRID, lw=1.4,
    ...
    if pub.is_on():
        ...
        ax.set_title("Rate predictor × outcome, Spearman ρ\n"
                     "* = survives Bonferroni within its grid", fontsize=9)
...
def fig6_arms(run: Path, out: Path, arm_dir: str, gains: dict, df: pd.DataFrame,
              spec: dict) -> None:
    s = pd.read_csv(run / arm_dir / "analysis_summary.csv")
    prim = spec["col"]
    if f"{prim}_ci" not in s.columns:
        sys.exit(f"analysis_summary.csv has no `{prim}_ci` -- it was written for a different site "
    ...
    blocks = [("primary", ["add_a1.0", "add_own", "random_a1.0"]),
    ...
    for title, conds in blocks:
        ...
        trans = ax.get_yaxis_transform()
        ax.text(-0.30, (top + y + 1) / 2, title, fontsize=7, color=GREY, ha="center",
                va="center", rotation=90, transform=trans, clip_on=False)
        ax.plot([-0.255, -0.255], [top + 0.32, y + 0.68], color=acs.GRID, lw=1.4,
                transform=trans, clip_on=False)
        y -= 1.0
    ...
    ax.set_title("Every stage-4 condition, bootstrap CI over genes  (n = 398).  "
                 "H2c is exploratory — its gate did not pass", fontsize=9)
    ...
    ax.errorbar(range(5), by.unsteered_pp.mean(), yerr=by.unsteered_pp.sem(), fmt="none",
                ecolor=GREY, lw=1)
    ax.set_xticks(range(5), STRATA, fontsize=7)
    ...
    fig.suptitle("G2 — the causal arms, the dose–response, and the headroom that explains the "
                 "conservation gradient", y=0.98)
...
def _scatter_rho(ax, x, y, xlab, ylab, title, resid_on=None, color=INK):
    ...
    ax.text(0.035, 0.955, f"ρ = {rho:+.3f}\n{pstr(p)}   n = {len(x)}", transform=ax.transAxes,
...
def fig7_conservation(run: Path, out: Path, df: pd.DataFrame, gains: dict,
                      scores: pd.DataFrame, spec: dict) -> None:
    ...
    m = m.merge((aa["add_a1.0"] - aa["unsteered"]).rename("d_aa").reset_index(),
                on="gene", how="left")
    c = m.perc_id_hp.to_numpy(float)
    XL = "human↔platypus protein identity (%)"

    fig, axes = plt.subplots(2, 4, figsize=(15.0, 7.2))
    _scatter_rho(axes[0, 0], c, m.loo_cos.to_numpy(float), XL,
                 "cos(D_i, v_-i)", "H1a orientation")
    _scatter_rho(axes[0, 1], c, m.loo_cos_pc1.to_numpy(float), XL,
                 "cos(D_i, v_-i), PC1 removed", "H1a orientation, PC1 removed")
    _scatter_rho(axes[0, 2], c, m.delta_norm.to_numpy(float), XL,
                 "||D_i||", "H1b magnitude")
    _scatter_rho(axes[0, 3], c, m.unsteered_pp.to_numpy(float), XL,
                 f"unsteered {spec['axis_level']}", "the READOUT's own baseline", color=PLUM)
    _scatter_rho(axes[1, 0], c, m.gain_pp.to_numpy(float), XL,
                 f"{spec['axis_delta'][:-5]}, add α=1 (pp)", "G2 steering gain, α = 1")
    _scatter_rho(axes[1, 1], c, m.gain_pp_own.to_numpy(float), XL,
                 f"{spec['axis_delta'][:-5]}, add α_i (pp)", "G2 steering gain, own-shift α")
    _scatter_rho(axes[1, 2], c, m.d_aa.to_numpy(float), XL,
                 "Δ amino-acid identity (pp)", "protein cost of steering")
    _scatter_rho(axes[1, 3], c, m.gain_pp.to_numpy(float), "conservation (rank residual)",
                 f"{spec['axis_delta'][:-5]}, α=1 (rank residual)",
                 "gain vs conservation, AFTER the baseline",
                 resid_on=m.unsteered_pp.to_numpy(float), color=WARM)
    for ax in axes[1, :3]:
        ax.axhline(0, color=GREY, ls=":", lw=1.0, zorder=0)
    axes[1, 3].axhline(0, color=GREY, ls=":", lw=1.0, zorder=0)
    fig.suptitle("Geometry and steering against the conservation axis — one point per gene, "
...
def fig8_strata(run: Path, out: Path, df: pd.DataFrame, scores: pd.DataFrame,
                spec: dict) -> None:
    ...
    """
    pairs = pd.read_csv(run / "stage1" / "pairs.csv")
    m = pairs.merge(df.drop(columns=[c for c in df.columns
                                     if c in pairs.columns and c != "gene"]),
                    on="gene", how="left")
...
def _draw_strata_panels(panels, m, spec, out, stem: str, ncols: int = 4) -> None:
    ...
    if pub.is_on():
        ...
        fig, axes = plt.subplots(nrow, ncols, squeeze=False,
                                 figsize=pub.size(pub.FULL, PUB_STRATA_ROW_H * nrow))
    else:
        fig, axes = plt.subplots(nrow, ncols, figsize=(PANEL_W * ncols, PANEL_H * nrow),
                                 squeeze=False)
    rs = np.random.default_rng(0)
    for ax, (col, ylab, title) in zip(axes.ravel(), panels):
        data = [m.loc[m.stratum == s, col].dropna().to_numpy(float) for s in range(5)]
        bp = ax.boxplot(data, positions=range(5), widths=0.62, showfliers=False,
        ...
        for s, d in enumerate(data):
            ax.scatter(s + rs.uniform(-0.16, 0.16, len(d)), d, s=3.2, color=acs.ANNOTATION,
        ...
        if col in READOUT_PANELS:
            ...
            if any(miss):
                ax.text(0.02, 0.98, "genes with no evidence, s0→s4:  "
                        + "/".join(str(x) for x in miss), transform=ax.transAxes,
                        fontsize=6.2, color=WARM, style="italic", va="top")
    ...
    title = ("Composition of each conservation stratum.  n = "
             ...
             + " for s0 (least conserved) through s4 (most conserved).  Boxes give the "
               ...
               "descriptive and carry no significance tests."
             + ("  " + note if note else ""))
...
def fig9_legacy(run: Path, out: Path, df: pd.DataFrame, gains: dict) -> None:
    ...
    m = df.merge(gains["add_a1.0"], on="gene", how="inner")
    rows = [("loo_cos", "orientation cos(D_i, v_-i)"),
            ("loo_cos_pc1", "orientation, PC1 removed"),
            ("delta_norm", "magnitude ||D_i||"),
    ...
    for r, (outc, ylab) in enumerate(rows):
        for c, pred in enumerate(LEGACY5):
            ...
            _scatter_rho(axes[r, c], m[pred].to_numpy(float), m[outc].to_numpy(float),
                         pred + ("   [confirmatory]" if promoted else ""),
                         ylab if c == 0 else "", pred if r == 0 else "",
    ...
    fig.suptitle("The five legacy tree statistics vs every outcome — one point per gene, "
...
def main() -> None:
    ...
    ap.add_argument("--only", nargs="+", type=int, choices=range(1, 10), default=list(range(1, 10)),
    ...
    if want & {2, 3, 6, 7, 8, 9}:
        ...
        scores, spec = load_scores(args.run, args.arm_dir, scores=args.scores,
                                   metric=args.metric, min_voters=args.min_voters)
        gains = gain_table(scores, spec)
        print(f"  {len(df)} genes with geometry + tree; "

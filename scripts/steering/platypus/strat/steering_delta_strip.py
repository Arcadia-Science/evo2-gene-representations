"""Steering performance in one panel: what steering buys, what it costs, and whether it stays coherent."""
...
from strat_metric import (  # noqa: E402
    DEFAULT_METRIC, METRICS, add_metric_args, coverage_note, load_scores,
)
...
def readouts(mspec: dict) -> list:
    """key, block, label, arrow gloss (+1 = up is better), how to derive it from the per-sample frame."""
    return [
        (SITE_KEY, "on-target", mspec["strip"], +1, None),
        ("aa_id_to_target", "on-target", "amino-acid identity\nto platypus", +1, None),
        ("indel_frac", "coherence", "indel burden\n(% of generated bp)", -1,
         lambda s: 100.0 * s.indel_bp / s.gen_bp),
        ("stop_density", "coherence", "premature stops\n(per 100 codons)", -1,
...
def load_deltas(run: Path, arm_dir: str, conditions: list[str], scale: str, *,
                scores: str | None = None, metric: str = DEFAULT_METRIC,
                min_voters: int = 1) -> tuple[pd.DataFrame, dict]:
    ...
    for key in keys:
        ...
        for cond in conditions:
            ...
            rows.append(pd.DataFrame({"gene": w.index, "metric": key, "condition": cond,
                                      "delta": delta.values, "baseline": base.values,
...
def violin(ax, x, y, half, face, disp, undisp, edge="white"):
    ...
    _, ug, d, _ = prof
    w = d / d.max() * half
    ax.fill_betweenx(undisp(ug), x - w, x + w, facecolor=face, edgecolor=edge, lw=0.7,
                     zorder=2, alpha=0.95)
...
def draw_gradient_violin(ax, x, geom, cmap, norm):
    ...
    yg, w, cloc = geom
    quads = [[(x - w[i], yg[i]), (x + w[i], yg[i]), (x + w[i + 1], yg[i + 1]),
              (x - w[i + 1], yg[i + 1])] for i in range(len(yg) - 1)]
    ...
    ax.plot(np.r_[x - w, (x + w)[::-1], x - w[0]], np.r_[yg, yg[::-1], yg[0]],
...
def main() -> None:
    ...
    ap.add_argument("--conditions", nargs="+", default=["add_a1.0"],
    ...
    ap.add_argument("--style", choices=["violin-gradient", "violin-strata", "violin", "swarm"],
    ...
    ap.add_argument("--yscale", choices=["symlog", "linear"], default="symlog",
                    help="symlog (default) is LINEAR through zero out to --linthresh and "
                         ...
                         "effect sizes")
    ap.add_argument("--linthresh", type=float, default=5.0,
    ...
    df, mspec = load_deltas(run, args.arm_dir, args.conditions, args.scale,
                            scores=args.scores, metric=args.metric, min_voters=args.min_voters)
    ...
    def disp(v):
        return np.arcsinh(np.asarray(v, float) / t) if args.yscale == "symlog" else np.asarray(v,
                                                                                               float)

    def undisp(u):
        return t * np.sinh(np.asarray(u, float)) if args.yscale == "symlog" else np.asarray(u, float)
    ...
    def summary_marker(xm, y, label=True, compact=False):
        ...
        ax.plot([xm - w * 0.8, xm + w * 0.8], [med, med], color=acs.apc.black, lw=1.1,
                ls=(0, (1.4, 1.0)), zorder=5)
        if label:
            sig = "" if (lo <= 0 <= hi) else "*"
            ax.text(xm + w + 0.06, mean, f"{mean:+.2f}{sig}", fontsize=8, color=acs.apc.black,
    ...
    for c in cols:
        ...
        if args.style == "swarm":
            ...
            inside = (y >= ylo) & (y <= yhi)
            ax.scatter(xs[inside], y[inside], s=26, c=sub.perc_id_hp.to_numpy(float)[inside],
                       cmap=CONS_CMAP, norm=norm, lw=0.45, edgecolor=acs.apc.white,
                       alpha=0.95 if not is_control else 0.55, zorder=2)
            for mask, edge, mark in ((y < ylo, ylo, "v"), (y > yhi, yhi, "^")):
                off = mask & np.isfinite(y)
                n_off += int(off.sum())
                if off.any():
                    ax.scatter(xs[off], np.full(off.sum(), edge), s=22, marker=mark,
                               c=sub.perc_id_hp.to_numpy(float)[off], cmap=CONS_CMAP, norm=norm,
                               lw=0.45, edgecolor="white", zorder=3, clip_on=False)
            summary_marker(c["x"] + 0.54, y)

        elif args.style == "violin":
            y = sub.delta.to_numpy(float)
            violin(ax, c["x"], y, 0.34, CONS_CMAP(0.45) if not is_control else acs.apc.denim,
                   disp, undisp)
    ...
    if args.style == "violin-gradient":
        ...
        for c in cols:
            sub = df[(df.metric == c["metric"]) & (df.condition == c["cond"])]
            g = gradient_profile(sub.delta.to_numpy(float), sub.perc_id_hp.to_numpy(float),
                                 0.34, disp, undisp)
            if g is not None:
        ...
        for c in cols:
            ...
            summary_marker(c["x"] + 0.46,
                           df[(df.metric == c["metric"])
                              & (df.condition == c["cond"])].delta.to_numpy(float))
    ...
    if args.yscale == "symlog":
        ...
        ax.text(_refx, t, f"log beyond ±{t:g}", transform=ax.get_yaxis_transform(), ha=_refha,
                va="bottom", fontsize=6.2, color=acs.ANNOTATION, zorder=6,
                bbox=dict(boxstyle="square,pad=0.12", fc="white", ec="none"))

    ax.axhline(0, color=GREY, ls=":", lw=1.1, zorder=1)
    _ux, _uha = (0.998, "right") if pub.is_on() else (0.006, "left")
    ax.text(_ux, 0.0, "unsteered", transform=ax.get_yaxis_transform(), ha=_uha, va="bottom",
            fontsize=6.5, color=GREY, zorder=6,
            bbox=dict(boxstyle="square,pad=0.12", fc="white", ec="none"))
    ...
    if len(conds) > 1:
        ...
        for key in keys:
            ...
            ax.text(np.mean(xs_), -0.125, f"{spec[key][1]}\n{better(key)}",
                    transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=8,
                    color=INK)
    else:
        ax.set_xticklabels([f"{spec[c['metric']][1]}\n{better(c['metric'])}" for c in cols],
                           fontsize=8)
    ...
    for blk in dict.fromkeys(c["block"] for c in cols):
        ...
        ax.plot([lo_, hi_], [ytxt + 0.035] * 2, color=acs.GRID, lw=1.2,
                transform=ax.get_xaxis_transform(), clip_on=False)
        ax.text(np.mean([lo_, hi_]), ytxt, blk, transform=ax.get_xaxis_transform(), ha="center",
                va="top", fontsize=8.5, color=GREY, style="italic")
    ...
    if args.style == "violin-strata":
        ...
        body = [[STRATUM_LAB[int(st)],
                 f"{edges.loc[st, 'min']:.0f}–{edges.loc[st, 'max']:.0f} %",
                 f"{int(edges.loc[st, 'nunique'])}"] for st in strata]
        if pub.is_on():
            # Use color patches for the equal-sized conservation strata.
            band_key = [Patch(facecolor=CONS_CMAP(STRATUM_POS[int(st)]), edgecolor=acs.apc.white,
                              label=f"{STRATUM_LAB[int(st)]} "
                                    f"({edges.loc[st, 'min']:.0f}–{edges.loc[st, 'max']:.0f}%)")
                        for st in strata]
            pub_extra_key = ("Conservation band — human vs platypus protein identity", band_key)
        else:
            tab = ax.table(cellText=body, colLabels=["stratum", "protein identity", "n genes"],
                           cellLoc="center", bbox=[0.425, -0.60, 0.15, 0.28])
            ...
            for (r, c), cell in tab.get_celld().items():
                ...
                if r == 0:
                    ...
                    cell.get_text().set_color("white" if STRATUM_POS[int(strata[r - 1])] > 0.5
                                              else acs.apc.black)
            ax.text(0.5, -0.305, "conservation band — human↔platypus protein identity",
                    transform=ax.transAxes, ha="center", va="bottom", fontsize=7.0, color=GREY)
    elif args.style == "violin":
        ...
        # what keeps the stretched ramp honest: it shows the relationship is weak, it doesn't hide it.
        grad = args.style == "violin-gradient"
        cb = fig.colorbar(plt.cm.ScalarMappable(norm=grad_norm if grad else norm, cmap=CONS_CMAP),
                          ax=ax, pad=0.045, fraction=0.036)
        cb.set_label(("local mean human↔platypus protein identity (%)\nof the genes at that height"
                      if grad else "human↔platypus protein identity (%)  →  more conserved"),
                     fontsize=7.0 if grad else 7.5)
        cb.ax.tick_params(labelsize=6.5)
        cb.outline.set_visible(False)

    marks = [Line2D([], [], color=acs.apc.black, lw=2.4,
                    label="mean, 95 % bootstrap CI over genes"),
             Line2D([], [], color=acs.apc.black, lw=1.1, ls=(0, (1.4, 1.0)), label="median")]
    if args.style == "swarm":
        marks.append(Line2D([], [], marker="o", ls="none", mfc=acs.apc.vital, mec=acs.apc.white,
                            mew=0.45,
                            ms=5.5, label=f"one gene (n = {n_genes})"))
    else:
        marks.append(Patch(facecolor=acs.apc.denim, edgecolor=acs.apc.white,
                           label=f"KDE over genes (n = {n_genes})"))
    style_tag = {"violin-gradient": "_violin_gradient", "violin-strata": "_violin_strata",
                 "violin": "_violin", "swarm": ""}[args.style]
    stem = args.stem or (f"10_steering_delta{style_tag}"
                         + ("" if args.scale == "pp" else "_relative"))
    if pub.is_on():
        ...
        ax.legend(handles=marks, frameon=False, loc="lower left", bbox_to_anchor=(0.0, 1.005),
                  ncol=len(marks), handletextpad=0.6, columnspacing=1.6)
        title, extra = pub_extra_key if pub_extra_key else ("Key", [])
        band_legend, key_h = pub.key_below(fig, extra, [h.get_label() for h in extra],
                                           title=title, width=pub.FULL, cols=len(extra))
        ...
        band_legend.set_bbox_to_anchor((pub.arc.MARGIN / pub.FULL, pub.arc.MARGIN / PUB_FIG_H),
        ...
        fig.subplots_adjust(left=PUB_LEFT / pub.FULL, right=1.0 - pub.arc.MARGIN / pub.FULL,
                            bottom=(key_h + PUB_BELOW_AXES) / PUB_FIG_H,
                            top=1.0 - PUB_TOP / PUB_FIG_H)
        pub.finish(fig, stem, directory=out)
        plt.close(fig)
    else:
        ax.legend(handles=marks, fontsize=6.8, frameon=False, loc="lower left",
                  bbox_to_anchor=(0.0, 1.005), ncol=3, handletextpad=0.6, columnspacing=1.4)
    ...
    if not pub.is_on():
        fig.suptitle(f"Steering buys on-target base pairs, costs little protein, and does not "
                     f"decohere the sequence  ({arm}, {args.arm_dir.replace('stage4_', '')})\n"
                     + coverage_note(mspec), y=0.995, fontsize=9.5)
        notes = ["Every readout is a percentage of something in the generated sequence, so every "
                 "column is percentage points on one axis."]
        if args.yscale == "symlog":
            notes.append(f"y is symlog: linear within ±{t:g}, log beyond, so equal vertical "
                         "distances are NOT equal deltas across the dashed break.")
        if n_off:
            notes.append(f"{n_off} gene(s) past the axis are drawn as carets and are still in "
                         "every statistic.")
        ...
        if args.notes:
            fig.text(0.5, -0.13 if args.style == "violin-strata" else -0.075, " ".join(notes),
    ...
    for c in cols:
        ...
        for st in [None, *strata]:
            ...
            mean, lo, hi = paired_bootstrap(y, args.n_boot, args.seed)
            row = {"metric": c["metric"], "condition": c["cond"],
                   "stratum": "all" if st is None else int(st),
                   "n": int(np.isfinite(y).sum()), "mean": round(mean, 3),
            ...
            if st is None:
                ...
                row |= {"rho_conservation": round(rho, 3), "p_conservation": p,
                        "rho_baseline": round(rho_b, 3), "p_baseline": p_b}
            rec.append(row)

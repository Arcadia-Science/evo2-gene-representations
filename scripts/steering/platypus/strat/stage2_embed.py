...
class Pooler:
    ...
    def set_window(self, keep_from: int, local_idx: np.ndarray,
                   abs_lo: int = 0, half_from: int = 0) -> None:
        ...
        self.idx = (torch.as_tensor(local_idx, device="cuda", dtype=torch.long)
                    if len(local_idx) else None)
    ...
    def pooled(self) -> dict[str, np.ndarray]:
        ...
        half = (np.stack([(self.hsum[li] / self.n_half).cpu().numpy() for li in range(NB)])
                if self.n_half else np.full_like(cds, np.nan))
        return {"cds_mean": cds, "aligned_mean": aln, "cds_second_half": half}


def embed_one(model, pool: Pooler, seq: str, aln_idx: np.ndarray, window: int,
              overlap: int) -> tuple[dict[str, np.ndarray], int]:
...
def main() -> None:
    ...
    for g in genes:
        ...
        idx[g] = (hi, ti)
        cov.append({"gene": g, "n_aligned_bp": len(hi), "cds_len_human": len(ch[g]),
    ...
    log("retained_frac: min %.3f med %.3f max %.3f" % (
        covdf.retained_frac.min(), covdf.retained_frac.median(), covdf.retained_frac.max()))
    ...
    with Pooler(model) as pool:
        for gi, g in enumerate(genes):
            for si, sp in enumerate(SPECIES):
                ...
                for m in MODES:
                    acts[m][gi, si] = out[m]
                norms.append({"gene": g, "species": sp, "n_pooled": pool.n,
                              "n_aligned": pool.n_aln, "n_windows": n_win, "cds_len": len(seq),
                              **{f"norm_b27_{m}": float(np.linalg.norm(out[m][27]))
                                 for m in MODES}})
                torch.cuda.empty_cache()
            if (gi + 1) % 20 == 0:
    ...
    np.savez_compressed(args.out / "pooled_representations.npz",
                        genes=np.array(genes), species=np.array(SPECIES),
                        **{m: acts[m] for m in MODES})
    pd.DataFrame(norms).to_csv(args.out / "pooled_norms.csv", index=False)
    (args.out / "stage2_config.json").write_text(json.dumps(
        {"n_genes": len(genes), "context_bp": args.context, "modes": MODES, "n_blocks": NB,
         "window_bp": args.window, "overlap_bp": args.overlap,
         "n_genes_windowed": int(sum(1 for r in norms if r["n_windows"] > 1) / 2),
         "retained_frac": [float(covdf.retained_frac.min()), float(covdf.retained_frac.median()),
                           float(covdf.retained_frac.max())]}, indent=2))

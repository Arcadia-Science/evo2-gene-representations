...
H_COL, P_COL, GEN_COL, STEER_COL = (
    acs.STEER_COLORS["source"],
    acs.STEER_COLORS["target"],
    acs.STEER_COLORS["unsteered"],
    acs.STEER_COLORS["steered"],
)
EXT = ("png", "pdf")
...
DOSE_SHADE = {
    "add_a0.5": acs.apc.sky,
    "add_a1.0": acs.apc.vital,
    "add_a2.0": acs.apc.aegean,
    "add_a3.0": acs.apc.lapis,
    "add_a4.0": acs.apc.dusk,
...
def fig21(per: pd.DataFrame, out: Path) -> pd.DataFrame:
    ...
    for ax, (col, title, scale) in zip(axes, SCALAR_PANELS, strict=False):
        data, keep = [], []
        for c in conds:
        ...
        for b, c in zip(parts["bodies"], keep, strict=False):
            b.set_facecolor(colour_for(c))
            b.set_alpha(0.45)
            b.set_edgecolor(colour_for(c))
            b.set_linewidth(0.6)
        ax.scatter(
            range(1, len(data) + 1), [np.mean(v) for v in data], s=9, color="black", zorder=5
        )
    ...
    for ax in axes[len(SCALAR_PANELS) :]:
        ax.set_visible(False)
    fig.suptitle(
        "Composition of the generations against the real human and platypus CDS windows\n"
    ...
    for e in EXT:
        fig.savefig(
            out / f"21_composition_vs_reference_distributions.{e}", dpi=300, bbox_inches="tight"
        )
    plt.close(fig)

    tab = (
        per[per.condition.isin(conds)]
        .groupby("condition")[[c for c, _t, _s in SCALAR_PANELS]]
        .mean()
        .reindex(conds)
    )
...
def fig22(dn: pd.DataFrame, out: Path) -> pd.DataFrame:
    ...
    for i, c in enumerate(conds):
        ax.bar(
            ...
            dn[c],
            width=w,
            label=LADDER_LABELS.get(c, c),
            color=colour_for(c),
            alpha=0.9 if c in (HUMAN_REF, PLAT_REF) else 0.75,
            edgecolor="white",
            lw=0.3,
        )
    ax.axhline(1.0, color=acs.ANNOTATION, lw=0.8, ls=":")
    ax.annotate(
        "ρ = 1: exactly as often as base composition predicts",
        (0.005, 1.0),
    ...
    ax.set_title(
        "Dinucleotide relative abundance — GC-shift-corrected dinucleotide structure\n"
...
def fig23(js: pd.DataFrame, per: pd.DataFrame, out: Path) -> pd.DataFrame:
    ...
    names = {
        "dinuc": "dinucleotides (16)",
        "kmer4": "4-mers (256)",
        "codon": "codon usage (61)",
    ...
    for ax, fam in zip(axes, fams, strict=False):
        ...
        for i, (c, xx, yy) in enumerate(zip(keep, xs, ys, strict=False)):
        ...
        ax.annotate(
            f"nearest generation is {far:.1f}× JSD(H,P) from human",
            (0.03, 0.97),
    ...
    fig.suptitle(
        "Distance to each species, per gene's own windows — below the diagonal = closer to "
        "platypus",
        fontsize=9,
    )
    ...
    plt.close(fig)
    t = js[js.condition.isin(conds)][
        ["family", "condition", "dH_mean", "dP_mean", "dP_minus_dH", "HP_mean", "n_genes"]
    ]
...
def fig24(sub: pd.DataFrame, per: pd.DataFrame, out: Path) -> pd.DataFrame:
    ...
    set_pub_style(title_size=9, tick_size=7)
    fig, axes = plt.subplots(1, 2, figsize=(10.6, 3.6), gridspec_kw={"width_ratios": [2.5, 1]})
    ax = axes[0]
    ...
    for i, c in enumerate(conds):
        ax.bar(
            ...
            label=LADDER_LABELS.get(c, c),
            color=colour_for(c),
            alpha=0.9 if c == PLAT_REF else 0.75,
            edgecolor="white",
            lw=0.3,
        )
    ...
    ax.set_title(
        "Substitution spectrum against the human window\n"
...
def main() -> None:
    ...
    print(
        t23.pivot(index="condition", columns="family", values=["dH_mean", "dP_mean"]).to_string(
